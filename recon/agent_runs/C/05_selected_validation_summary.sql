...
SELECT
  ...
  candidate_rule,
  COUNT(*) AS card_count,
  SUM(CASE WHEN wa_final_days IS NOT NULL THEN 1 ELSE 0 END) AS wa_days_present,
  SUM(CASE WHEN candidate_days IS NOT NULL THEN 1 ELSE 0 END) AS candidate_days_present,
  SUM(CASE WHEN wa_final_days IS NOT NULL AND candidate_days IS NOT NULL AND wa_final_days = candidate_days THEN 1 ELSE 0 END) AS exact_day_count,
  ROUND(SUM(CASE WHEN wa_final_days IS NOT NULL AND candidate_days IS NOT NULL AND wa_final_days = candidate_days THEN 1 ELSE 0 END) / COUNT(*), 6) AS exact_day_rate,
  SUM(CASE WHEN wa_final_days IS NOT NULL AND candidate_days IS NOT NULL AND wa_final_days <> candidate_days AND ABS(wa_final_days - candidate_days) = 1 THEN 1 ELSE 0 END) AS day_error_1_count,
  SUM(CASE WHEN wa_final_days IS NOT NULL AND candidate_days IS NOT NULL AND ABS(wa_final_days - candidate_days) >= 2 THEN 1 ELSE 0 END) AS day_error_ge2_count,
  SUM(CASE WHEN platform_package_price_count IS NOT NULL AND platform_package_price_count > 0 AND package_price_exact_match = 1 THEN 1 ELSE 0 END) AS exact_platform_price_count,
  ROUND(SUM(CASE WHEN platform_package_price_count IS NOT NULL AND platform_package_price_count > 0 AND package_price_exact_match = 1 THEN 1 ELSE 0 END) / COUNT(*), 6) AS exact_platform_price_rate,
  SUM(CASE WHEN calculated_amount IS NOT NULL AND ABS(COALESCE(wa_amount, 0) - calculated_amount) <= 0.01 THEN 1 ELSE 0 END) AS amount_exact_count,
  ROUND(SUM(CASE WHEN calculated_amount IS NOT NULL AND ABS(COALESCE(wa_amount, 0) - calculated_amount) <= 0.01 THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN calculated_amount IS NOT NULL THEN 1 ELSE 0 END), 0), 6) AS amount_exact_rate_on_evaluable,
  SUM(CASE WHEN wa_final_days IS NULL OR candidate_days IS NULL THEN 1 ELSE 0 END) AS unresolved_day_count,
  SUM(CASE WHEN platform_package_price_count IS NULL OR platform_package_price_count = 0 THEN 1 ELSE 0 END) AS missing_platform_price_count,
  ROUND(SUM(wa_amount), 6) AS wa_amount_sum,
  ROUND(SUM(CASE WHEN calculated_amount IS NOT NULL THEN calculated_amount ELSE 0 END), 6) AS calculated_amount_sum
FROM calculated
