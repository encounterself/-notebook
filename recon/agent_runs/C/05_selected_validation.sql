...
SELECT
  ...
  wa_row_count,
  wa_final_days,
  wa_final_days_values,
  candidate_rule,
  candidate_days,
  wa_price,
  wa_price_values,
  platform_package_price_count,
  platform_package_prices,
  package_price_exact_match,
  platform_product_name_count,
  platform_product_names,
  wa_amount,
  calculated_amount,
  CASE WHEN calculated_amount IS NOT NULL THEN ROUND(wa_amount - calculated_amount, 6) END AS amount_diff,
  CASE WHEN wa_final_days IS NULL OR candidate_days IS NULL THEN 'MISSING_DATA'
       WHEN wa_final_days <> candidate_days THEN 'UNRESOLVED_DAY_RULE'
       WHEN charge_type LIKE 'Credit%' THEN 'CREDIT_SIGN_PRESERVED_NO_AMOUNT_INFERENCE'
       WHEN platform_package_price_count IS NULL OR platform_package_price_count = 0 THEN 'MISSING_DATA'
       WHEN package_price_exact_match <> 1 THEN 'MISSING_MAPPING'
       WHEN ABS(wa_amount - calculated_amount) > 0.01 THEN 'UNRESOLVED_AMOUNT'
       ELSE 'EXACT_WA_DAY_PRICE_AMOUNT'
  END AS validation_status,
  cycle_row_count,
  next_cycle_date_count,
  next_cycle_dates,
  cycle_next_first_utc,
  cycle_next_first_minus5,
  platform_cycle_to_month_end_days,
  status_log_row_count,
  activation_first_utc,
  activation_first_minus5,
  platform_activation_to_month_end_days,
  void_first_utc,
  void_first_minus5,
  platform_final_start_to_void_minus1_days,
  wa_source_files,
  wa_sheet_names
FROM calculated
ORDER BY billing_month, charge_type, imsi;