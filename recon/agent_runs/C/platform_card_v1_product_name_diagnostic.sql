...
excel_detail_raw AS (
  SELECT
    ...
    trim(product_name) AS excel_product_name, trim(charge_type) AS charge_type,
...
SELECT
  charge_type,
  CASE
    WHEN platform_product_id IS NULL THEN 'NO_PLATFORM_PRODUCT_ID'
    WHEN excel_product_name IS NULL OR excel_product_name = '' THEN 'EXCEL_PRODUCT_NAME_NULL'
    WHEN excel_product_name = platform_product_name THEN 'EXACT_PRODUCT_NAME'
    ELSE 'PRODUCT_NAME_MISMATCH'
  END AS product_name_mapping_diagnostic,
  COUNT(*) AS rows,
  COUNT(DISTINCT imsi) AS imsi_count,
  SUM(excel_amount) AS excel_total,
  SUM(CASE WHEN source_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS current_matched_count
FROM finalized
GROUP BY charge_type,
  CASE
    ...
    ELSE 'PRODUCT_NAME_MISMATCH'
  END
ORDER BY charge_type, product_name_mapping_diagnostic