...
SELECT billing_month, charge_type, source_layer, candidate_rule, candidate_status,
  COUNT(*) AS row_count, COUNT(DISTINCT imsi) AS imsi_count,
  ROUND(SUM(raw_wa_amount), 6) AS raw_wa_amount_sum,
  ROUND(SUM(CASE WHEN candidate_amount IS NOT NULL THEN candidate_amount ELSE 0 END), 6) AS candidate_amount_sum,
  SUM(CASE WHEN candidate_amount IS NOT NULL THEN 1 ELSE 0 END) AS candidate_amount_rows
FROM (
...
GROUP BY billing_month, charge_type, source_layer, candidate_rule, candidate_status
ORDER BY billing_month, source_layer, charge_type, candidate_status