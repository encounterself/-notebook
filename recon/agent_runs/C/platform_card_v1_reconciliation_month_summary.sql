...
SELECT
  ...
  count(DISTINCT CASE WHEN record_side = 'EXCEL_DETAIL' THEN imsi END) AS excel_imsi_count,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN excel_amount ELSE CAST(NULL AS DECIMAL(38,10)) END) AS excel_total,
  ...
  sum(CASE WHEN source_match_status = 'PLATFORM_ONLY' THEN 1 ELSE 0 END) AS platform_only_count,
