...
recon_joined AS (
  SELECT
    ...
    CASE
      ...
      WHEN p.platform_fact_key IS NULL THEN e.difference_reason
      ELSE e.difference_reason
    END AS difference_reason,
    CASE
      WHEN e.excel_row_key IS NULL THEN 'PLATFORM_ONLY'
      WHEN p.platform_fact_key IS NULL THEN e.source_match_status
      ELSE e.source_match_status
...
SELECT
  ...
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN platform_calculated_amount ELSE CAST(NULL AS DECIMAL(38,10)) END) AS platform_calculated_total,
  ...
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN excel_days ELSE CAST(NULL AS DECIMAL(38,10)) END) AS excel_days_total,
  ...
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN day_diff ELSE CAST(NULL AS DECIMAL(38,10)) END) AS day_diff_total,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN abs(day_diff) ELSE CAST(NULL AS DECIMAL(38,10)) END) AS day_abs_diff_total,
  ...
  count(DISTINCT CASE WHEN source_match_status = 'MATCHED' THEN imsi END) AS matched_distinct_imsi_count,
  ...
  sum(CASE WHEN source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN excel_amount ELSE CAST(NULL AS DECIMAL(38,10)) END) AS unresolved_excel_total,
  sum(CASE WHEN source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN platform_calculated_amount ELSE CAST(NULL AS DECIMAL(38,10)) END) AS unresolved_platform_total,
  sum(CASE WHEN source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN amount_diff ELSE CAST(NULL AS DECIMAL(38,10)) END) AS unresolved_amount_diff_total,
  sum(CASE WHEN source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN abs(amount_diff) ELSE CAST(NULL AS DECIMAL(38,10)) END) AS unresolved_abs_amount_diff_total
FROM recon_joined
