...
SELECT
  ...
  charge_type,
  CASE
    WHEN charge_type = 'Full Cycle Charge' THEN 'R_FULL_CYCLE_MONTHLY'
    WHEN charge_type = 'Full Cycle Charge - Backbilled for FEB Invoice' THEN 'R_FULL_CYCLE_BACKBILLED_MONTHLY'
    WHEN charge_type = 'New Activation: Prorated-in Charge' THEN 'R_DAILY_PRORATED_NEW_ACTIVATION'
    WHEN charge_type = 'Partial Charge - Card Replaced Mid Cycle' THEN 'R_DAILY_PRORATED_REPLACEMENT_OLD_CARD'
    WHEN charge_type = 'Partial Charge - New Card Used as Replacement Mid Cycle' THEN 'R_DAILY_PRORATED_REPLACEMENT_NEW_CARD'
    WHEN charge_type = 'Credit for Offstocked Card' THEN 'R_DAILY_CREDIT_OFFSTOCK'
    WHEN charge_type = concat('Prorated Out - Transferred to Wing', chr(39), 's Simbank') THEN 'R_DAILY_PRORATED_SIMBANK_TRANSFER'
    WHEN charge_type = 'Prorated-out Charge - Offstocked Card' THEN 'R_DAILY_PRORATED_OFFSTOCK'
    WHEN charge_type = 'Offstocked before cycle start' THEN 'R_OFFSTOCK_BEFORE_CYCLE_UNRESOLVED'
    ELSE 'R_UNSUPPORTED_CHARGE_TYPE'
  END AS rule_id,
  count(*) AS excel_row_count,
  count(DISTINCT imsi) AS excel_imsi_count,
  sum(excel_amount) AS excel_total,
  sum(platform_calculated_amount) AS platform_calculated_total,
  ...
  sum(day_diff) AS day_diff_total,
  sum(abs(day_diff)) AS day_abs_diff_total,
  ...
  sum(CASE WHEN source_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS matched_count,
  sum(CASE WHEN source_match_status = 'WA_ONLY' THEN 1 ELSE 0 END) AS wa_only_count,
  sum(CASE WHEN source_match_status = 'UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_count,
  sum(CASE WHEN source_match_status = 'MISSING_DATA' THEN 1 ELSE 0 END) AS missing_data_count,
  sum(CASE WHEN source_match_status = 'MISSING_MAPPING' THEN 1 ELSE 0 END) AS missing_mapping_count,
  sum(CASE WHEN abs(day_diff) <= 0.0001 THEN 1 ELSE 0 END) AS day_exact_count,
  sum(CASE WHEN abs(price_diff) <= 0.0001 THEN 1 ELSE 0 END) AS price_exact_count,
  sum(CASE WHEN abs(amount_diff) <= 0.0001 THEN 1 ELSE 0 END) AS amount_exact_count,
  ...
  sum(CASE WHEN source_match_status = 'MATCHED' THEN 1 ELSE 0 END) / nullif(count(*),0) AS matched_rate,
  count(platform_calculated_days) / nullif(count(*),0) AS days_population_rate,
  count(platform_calculated_price) / nullif(count(*),0) AS price_population_rate,
  count(platform_calculated_amount) / nullif(count(*),0) AS amount_population_rate,
  sum(CASE WHEN abs(day_diff) <= 0.0001 THEN 1 ELSE 0 END) / nullif(count(*),0) AS day_exact_rate,
  sum(CASE WHEN abs(price_diff) <= 0.0001 THEN 1 ELSE 0 END) / nullif(count(*),0) AS price_exact_rate,
  sum(CASE WHEN abs(amount_diff) <= 0.0001 THEN 1 ELSE 0 END) / nullif(count(*),0) AS amount_exact_rate
FROM finalized
GROUP BY official_excel_billing_month, charge_type
