...
, timezone_scored AS (
  SELECT calculated.*,
    CASE WHEN charge_type = 'Full Cycle Charge' THEN CAST(month_days AS DOUBLE)
         WHEN charge_type IN ('Full Cycle Charge - Backbilled for FEB Invoice') AND excel_final_start_date IS NOT NULL AND excel_final_end_date IS NOT NULL THEN CAST(DATEDIFF(excel_final_end_date, excel_final_start_date) + 1 AS DOUBLE)
         WHEN charge_type = 'New Activation: Prorated-in Charge' AND activation_utc IS NOT NULL THEN CAST(DATEDIFF(d2, TO_DATE(activation_utc - INTERVAL 5 HOURS)) + 1 AS DOUBLE)
         WHEN charge_type LIKE 'Partial Charge%' AND void_utc IS NOT NULL AND excel_final_start_date IS NOT NULL THEN CAST(DATEDIFF(DATE_SUB(TO_DATE(void_utc - INTERVAL 5 HOURS), 1), excel_final_start_date) + 1 AS DOUBLE)
         WHEN charge_type LIKE 'Prorated Out%' OR charge_type LIKE 'Prorated-out%' THEN CASE WHEN cycle_boundary_minus5 IS NOT NULL THEN CAST(DATEDIFF(d2, TO_DATE(cycle_boundary_minus5)) + 1 AS DOUBLE) END
         ELSE NULL END AS platform_calculated_days_minus5
  FROM calculated
)
SELECT billing_month, charge_type, COUNT(*) AS card_count,
  SUM(CASE WHEN excel_final_days IS NOT NULL AND platform_calculated_days IS NOT NULL THEN 1 ELSE 0 END) AS utc_evaluable,
  SUM(CASE WHEN excel_final_days IS NOT NULL AND platform_calculated_days IS NOT NULL AND excel_final_days = platform_calculated_days THEN 1 ELSE 0 END) AS utc_exact,
  SUM(CASE WHEN excel_final_days IS NOT NULL AND platform_calculated_days_minus5 IS NOT NULL THEN 1 ELSE 0 END) AS minus5_evaluable,
  SUM(CASE WHEN excel_final_days IS NOT NULL AND platform_calculated_days_minus5 IS NOT NULL AND excel_final_days = platform_calculated_days_minus5 THEN 1 ELSE 0 END) AS minus5_exact,
  SUM(CASE WHEN platform_calculated_days IS NOT NULL AND platform_calculated_days_minus5 IS NOT NULL AND platform_calculated_days <> platform_calculated_days_minus5 THEN 1 ELSE 0 END) AS timezone_days_disagreement,
  SUM(CASE WHEN platform_calculated_days IS NOT NULL AND platform_calculated_days_minus5 IS NOT NULL AND excel_final_days IS NOT NULL AND ABS(excel_final_days - platform_calculated_days) <= 0.01 AND ABS(excel_final_days - platform_calculated_days_minus5) > 0.01 THEN 1 ELSE 0 END) AS utc_only_day_exact,
  SUM(CASE WHEN platform_calculated_days IS NOT NULL AND platform_calculated_days_minus5 IS NOT NULL AND excel_final_days IS NOT NULL AND ABS(excel_final_days - platform_calculated_days_minus5) <= 0.01 AND ABS(excel_final_days - platform_calculated_days) > 0.01 THEN 1 ELSE 0 END) AS minus5_only_day_exact,
  SUM(CASE WHEN platform_calculated_price IS NOT NULL AND platform_calculated_days_minus5 IS NOT NULL THEN 1 ELSE 0 END) AS minus5_amount_evaluable,
  SUM(CASE WHEN platform_calculated_price IS NOT NULL AND platform_calculated_days_minus5 IS NOT NULL AND ABS(excel_final_charge - CASE WHEN charge_type IN ('Full Cycle Charge', 'Full Cycle Charge - Backbilled for FEB Invoice') THEN ROUND(platform_calculated_price,6) ELSE ROUND(platform_calculated_price * platform_calculated_days_minus5 / month_days,6) END) <= 0.01 THEN 1 ELSE 0 END) AS minus5_amount_exact
FROM timezone_scored
