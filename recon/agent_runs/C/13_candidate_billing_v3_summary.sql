...
, pending_raw AS (
  SELECT c.billing_month, CAST(x.imsi AS STRING) AS imsi,
    ROUND(SUM(TRY_CAST(x.final_charge_for_usage_days AS DOUBLE)), 6) AS excel_final_charge
  ...
  SELECT billing_month, imsi, charge_type,
    ...
    excel_final_days, platform_calculated_days, excel_minus_platform_days,
    excel_monthly_rate, platform_calculated_price, excel_minus_platform_price,
    excel_final_charge, platform_calculated_amount,
    ...
    history_price_count, dim_price_count
  FROM calculated
  UNION ALL
  SELECT billing_month, imsi, 'Pending Prorated-In', 'PENDING_MISSING_FINAL_DAYS',
    CAST(NULL AS DOUBLE), CAST(NULL AS DOUBLE), CAST(NULL AS DOUBLE), CAST(NULL AS DOUBLE), CAST(NULL AS DOUBLE), CAST(NULL AS DOUBLE),
    excel_final_charge, CAST(NULL AS DOUBLE), CAST(NULL AS DOUBLE), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT)
  FROM pending_raw
)
SELECT billing_month, charge_type, candidate_status,
  COUNT(*) AS card_count,
  ...
  SUM(CASE WHEN excel_minus_platform_days = 1 THEN 1 ELSE 0 END) AS platform_day_error_plus1,
  SUM(CASE WHEN ABS(excel_minus_platform_days) >= 2 THEN 1 ELSE 0 END) AS platform_day_error_ge2,
  SUM(CASE WHEN excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL THEN 1 ELSE 0 END) AS price_evaluable,
  SUM(CASE WHEN excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL AND ABS(excel_minus_platform_price) <= 0.01 THEN 1 ELSE 0 END) AS price_exact,
  SUM(CASE WHEN platform_calculated_amount IS NOT NULL THEN 1 ELSE 0 END) AS amount_evaluable,
  SUM(CASE WHEN platform_calculated_amount IS NOT NULL AND ABS(platform_amount_diff) <= 0.01 THEN 1 ELSE 0 END) AS amount_exact,
  ROUND(SUM(CASE WHEN platform_calculated_amount IS NOT NULL THEN platform_amount_diff ELSE 0 END), 6) AS amount_diff_sum,
  ROUND(SUM(excel_final_charge), 6) AS excel_amount_sum,
...
FROM all_candidate
GROUP BY billing_month, charge_type, candidate_status
ORDER BY billing_month, charge_type, candidate_status