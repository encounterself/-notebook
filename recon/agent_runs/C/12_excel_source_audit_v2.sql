WITH cfg AS (
  ...
  SELECT billing_month, COUNT(*) AS detail_raw_rows, COUNT(DISTINCT imsi) AS detail_distinct_imsi,
    COUNT(*) AS detail_canonical_rows,
    COUNT(DISTINCT imsi) AS detail_canonical_imsi, SUM(dedup_amount) AS detail_dedup_amount,
...
SELECT m.billing_month,
  m.detail_raw_rows, m.detail_distinct_imsi, m.detail_canonical_rows, m.detail_canonical_imsi, ROUND(m.detail_dedup_amount, 6) AS detail_dedup_amount,
