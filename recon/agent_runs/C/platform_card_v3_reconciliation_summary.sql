...
SELECT
  ...
  CASE WHEN record_side = 'EXCEL_DETAIL' AND count(*) = count(platform_calculated_amount_known)
             AND sum(CASE WHEN source_match_status = 'MATCHED' THEN 1 ELSE 0 END) = count(*) THEN 'COMPLETE' ELSE 'UNRESOLVED' END AS platform_total_status,
...
FROM recon_joined
GROUP BY reconciliation_billing_month, official_excel_billing_month, excel_source_file,
         date_format(excel_final_start, 'yyyy-MM'), source_match_status,
         numeric_match_status, record_side, charge_type
ORDER BY reconciliation_billing_month, record_side, charge_type, source_match_status