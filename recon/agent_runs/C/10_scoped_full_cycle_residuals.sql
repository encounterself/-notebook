...
SELECT billing_month, imsi, charge_type, excel_product_id, excel_product_names,
  excel_final_start_date, excel_final_end_date, excel_final_days, excel_final_days_values,
  excel_monthly_rate, excel_monthly_rate_values, excel_final_charge,
  ...
  CASE WHEN excel_final_days IS NOT NULL AND platform_calculated_days IS NOT NULL THEN excel_final_days - platform_calculated_days END AS platform_day_diff,
  platform_calculated_price,
  CASE WHEN excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL THEN excel_monthly_rate - platform_calculated_price END AS platform_price_diff,
  CASE WHEN charge_type LIKE 'Credit%' THEN NULL
       WHEN platform_calculated_price IS NOT NULL AND platform_calculated_days IS NOT NULL THEN CASE WHEN charge_type IN ('Full Cycle Charge', 'Full Cycle Charge - Backbilled for FEB Invoice') THEN ROUND(platform_calculated_price, 6) ELSE ROUND(platform_calculated_price * platform_calculated_days / month_days, 6) END
       ELSE NULL END AS platform_calculated_amount,
  CASE WHEN charge_type LIKE 'Credit%' THEN NULL
       WHEN platform_calculated_price IS NOT NULL AND platform_calculated_days IS NOT NULL THEN excel_final_charge - CASE WHEN charge_type IN ('Full Cycle Charge', 'Full Cycle Charge - Backbilled for FEB Invoice') THEN ROUND(platform_calculated_price, 6) ELSE ROUND(platform_calculated_price * platform_calculated_days / month_days, 6) END
       ELSE NULL END AS platform_amount_diff,
  cycle_boundary_utc, cycle_boundary_minus5, activation_utc, void_utc,
  status_snapshot_rows, cycle_rows, status_log_rows,
  CASE WHEN platform_calculated_days IS NULL THEN 'MISSING_PLATFORM_DAYS'
       WHEN excel_final_days <> platform_calculated_days THEN 'PLATFORM_DAY_RESIDUAL'
       WHEN platform_calculated_price IS NULL THEN 'MISSING_PLATFORM_PRICE'
       WHEN ABS(excel_monthly_rate - platform_calculated_price) > 0.01 THEN 'PLATFORM_PRICE_RESIDUAL'
       WHEN platform_calculated_price IS NOT NULL AND platform_calculated_days IS NOT NULL AND ABS(excel_final_charge - CASE WHEN charge_type IN ('Full Cycle Charge', 'Full Cycle Charge - Backbilled for FEB Invoice') THEN ROUND(platform_calculated_price, 6) ELSE ROUND(platform_calculated_price * platform_calculated_days / month_days, 6) END) > 0.01 THEN 'PLATFORM_AMOUNT_RESIDUAL'
       ELSE 'NO_RESIDUAL' END AS residual_reason
FROM calculated
WHERE charge_type NOT LIKE 'Credit%'
  AND (
    platform_calculated_days IS NULL
    OR (excel_final_days IS NOT NULL AND excel_final_days <> platform_calculated_days)
    OR platform_calculated_price IS NULL
    OR (excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL AND ABS(excel_monthly_rate - platform_calculated_price) > 0.01)
    OR (platform_calculated_price IS NOT NULL AND platform_calculated_days IS NOT NULL AND ABS(excel_final_charge - CASE WHEN charge_type IN ('Full Cycle Charge', 'Full Cycle Charge - Backbilled for FEB Invoice') THEN ROUND(platform_calculated_price, 6) ELSE ROUND(platform_calculated_price * platform_calculated_days / month_days, 6) END) > 0.01)
  )
AND charge_type IN ('Full Cycle Charge', 'Full Cycle Charge - Backbilled for FEB Invoice')
ORDER BY billing_month, charge_type, residual_reason, imsi
