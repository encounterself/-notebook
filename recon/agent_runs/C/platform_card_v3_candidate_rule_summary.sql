...
SELECT
  ...
  excel_source_file AS source_file,
  date_format(excel_final_start, 'yyyy-MM') AS observed_start_month,
  charge_type,
  rule_id,
  count(*) AS row_count,
  ...
  CASE WHEN count(*) = count(platform_calculated_amount_known)
             AND sum(CASE WHEN formal_match_status = 'MATCHED' THEN 1 ELSE 0 END) = count(*)
       THEN sum(platform_calculated_amount_known)
       ELSE CAST(NULL AS DECIMAL(38,10)) END AS platform_total,
  CASE WHEN count(*) = count(platform_calculated_amount_known)
             AND sum(CASE WHEN formal_match_status = 'MATCHED' THEN 1 ELSE 0 END) = count(*) THEN 'COMPLETE' ELSE 'UNRESOLVED' END AS platform_total_status,
  sum(platform_calculated_amount - excel_amount) AS candidate_amount_diff_total,
  sum(excel_days) AS excel_days_total,
  sum(platform_calculated_days) AS platform_days_total,
  sum(platform_calculated_days - excel_days) AS day_diff_total,
  count(platform_calculated_days) AS platform_days_population,
  count(platform_calculated_price) AS platform_price_population,
  count(platform_calculated_amount) AS platform_amount_population,
  ...
  sum(CASE WHEN formal_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS formal_matched_row_count,
  count(DISTINCT CASE WHEN formal_match_status = 'MATCHED' THEN imsi END) AS formal_matched_distinct_imsi_count,
  sum(CASE WHEN formal_match_status = 'NUMERIC_MATCH_ONLY' THEN 1 ELSE 0 END) AS numeric_match_only_row_count,
  count(DISTINCT CASE WHEN formal_match_status = 'NUMERIC_MATCH_ONLY' THEN imsi END) AS numeric_match_only_distinct_imsi_count,
  sum(CASE WHEN formal_match_status = 'UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_row_count,
  count(DISTINCT CASE WHEN formal_match_status = 'UNRESOLVED' THEN imsi END) AS unresolved_distinct_imsi_count,
  sum(CASE WHEN formal_match_status = 'MISSING_MAPPING' THEN 1 ELSE 0 END) AS missing_mapping_row_count,
  count(DISTINCT CASE WHEN formal_match_status = 'MISSING_MAPPING' THEN imsi END) AS missing_mapping_distinct_imsi_count,
  sum(CASE WHEN formal_match_status = 'MISSING_DATA' THEN 1 ELSE 0 END) AS missing_data_row_count,
  count(DISTINCT CASE WHEN formal_match_status = 'MISSING_DATA' THEN imsi END) AS missing_data_distinct_imsi_count,
  sum(CASE WHEN formal_match_status = 'WA_ONLY' THEN 1 ELSE 0 END) AS wa_only_row_count,
  count(DISTINCT CASE WHEN formal_match_status = 'WA_ONLY' THEN imsi END) AS wa_only_distinct_imsi_count,
  sum(CASE WHEN abs(platform_calculated_days - excel_days) <= 0.0001 THEN 1 ELSE 0 END) AS day_exact_row_count,
  sum(CASE WHEN abs(platform_calculated_price - excel_price) <= 0.0001 THEN 1 ELSE 0 END) AS price_exact_row_count,
  sum(CASE WHEN abs(platform_calculated_amount - excel_amount) <= 0.0001 THEN 1 ELSE 0 END) AS amount_exact_row_count
FROM finalized
GROUP BY official_excel_billing_month, excel_source_file, date_format(excel_final_start, 'yyyy-MM'), charge_type, rule_id
ORDER BY official_excel_billing_month, charge_type