...
platform_fact AS (
  ...
  COUNT(*) AS wing_alpha_platform_cards,
  ...
  SUM(CASE WHEN platform_product_id_count > 0 THEN 1 ELSE 0 END) AS product_id_mapped_cards,
  SUM(CASE WHEN dim_price_count = 1 THEN 1 ELSE 0 END) AS product_id_unique_price_cards,
  SUM(CASE WHEN dim_price_count > 1 THEN 1 ELSE 0 END) AS product_id_multiple_price_cards,
  SUM(CASE WHEN dim_price_count = 0 OR dim_price_count IS NULL THEN 1 ELSE 0 END) AS product_id_missing_price_cards
FROM platform_fact
