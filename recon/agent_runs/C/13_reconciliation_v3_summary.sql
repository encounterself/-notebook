...
recon AS (
  SELECT COALESCE(w.billing_month, p.billing_month) AS billing_month,
    ...
    'platform status partition + cycle window first filtered to WA product_id scope; status_log joined only to scoped keys and target partition' AS platform_scope_basis,
...
SELECT billing_month, reconciliation_status, platform_scope_status, platform_price_status, product_effective_status,
  COUNT(*) AS card_count,
  ROUND(SUM(COALESCE(excel_total_amount, 0)), 6) AS excel_total_amount_sum,
  SUM(CASE WHEN detail_rows > 0 THEN 1 ELSE 0 END) AS detail_cards,
  SUM(CASE WHEN pending_rows > 0 THEN 1 ELSE 0 END) AS pending_cards,
  SUM(CASE WHEN status_snapshot_rows > 0 THEN 1 ELSE 0 END) AS status_snapshot_cards,
  SUM(CASE WHEN cycle_rows > 0 THEN 1 ELSE 0 END) AS cycle_cards,
  SUM(CASE WHEN status_log_rows > 0 THEN 1 ELSE 0 END) AS status_log_cards,
  SUM(CASE WHEN dim_price_count = 1 THEN 1 ELSE 0 END) AS product_id_price_present_cards,
  SUM(CASE WHEN dim_price_count = 0 OR dim_price_count IS NULL THEN 1 ELSE 0 END) AS missing_product_id_price_cards
FROM recon
GROUP BY billing_month, reconciliation_status, platform_scope_status, platform_price_status, product_effective_status
ORDER BY billing_month, reconciliation_status, platform_scope_status, platform_price_status, product_effective_status