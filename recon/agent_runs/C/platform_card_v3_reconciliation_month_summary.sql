...
evidence_status AS (
  SELECT n.*,
         CASE
           WHEN n.platform_candidate_count = 0 THEN 'NO_PLATFORM_CARD_WINDOW'
           WHEN n.platform_candidate_product_count > 1 OR n.platform_candidate_window_count > 1 THEN 'MULTIPLE_PLATFORM_PRODUCT_OR_WINDOW_CANDIDATES'
           WHEN n.excel_final_start IS NULL OR n.excel_final_end IS NULL THEN 'NO_EXCEL_FINAL_WINDOW'
           WHEN n.platform_price_state <> 'ONE_PRICE' THEN 'PLATFORM_PRICE_NOT_UNIQUE'
           WHEN n.rule_id = 'R_DAILY_PRORATED_NEW_ACTIVATION' AND n.activation_event_count = 0 THEN 'NO_ACTIVATION_EVENT_IN_PLATFORM_LOG'
           WHEN n.rule_id IN ('R_DAILY_PRORATED_OFFSTOCK','R_DAILY_CREDIT_OFFSTOCK') AND n.offstock_event_count = 0 THEN 'NO_OFFSTOCK_EVENT_IN_PLATFORM_LOG'
           WHEN n.rule_id IN ('R_DAILY_PRORATED_REPLACEMENT_OLD_CARD','R_DAILY_PRORATED_REPLACEMENT_NEW_CARD') AND n.replacement_event_count = 0 THEN 'NO_EXPLICIT_REPLACEMENT_MAPPING'
           WHEN n.rule_id = 'R_DAILY_PRORATED_SIMBANK_TRANSFER' AND n.transfer_event_count = 0 THEN 'NO_EXPLICIT_SIMBANK_TRANSFER_MAPPING'
           WHEN n.rule_id IN ('R_OFFSTOCK_BEFORE_CYCLE_UNRESOLVED','R_UNSUPPORTED_CHARGE_TYPE') THEN 'RULE_NOT_PROVABLE'
           WHEN n.platform_calculated_amount IS NULL THEN 'NO_PLATFORM_AMOUNT'
           ELSE 'KNOWN'
...
finalized AS (
  SELECT e.*,
         CASE
           ...
           WHEN e.platform_price_state <> 'ONE_PRICE' THEN 'PLATFORM_PRICE_NOT_UNIQUE'
         ...
         CASE
           ...
           WHEN e.platform_amount_evidence_status IN ('NO_EXCEL_FINAL_WINDOW','PLATFORM_PRICE_NOT_UNIQUE','NO_PLATFORM_AMOUNT','NO_ACTIVATION_EVENT_IN_PLATFORM_LOG','NO_OFFSTOCK_EVENT_IN_PLATFORM_LOG') THEN 'MISSING_DATA'
           WHEN e.platform_amount_evidence_status = 'RULE_NOT_PROVABLE' THEN 'UNRESOLVED'
           WHEN e.excel_product_id IS NULL AND e.numeric_match_status = 'NUMERIC_MATCH' THEN 'NUMERIC_MATCH_ONLY'
           WHEN e.excel_product_id IS NULL THEN 'MISSING_MAPPING'
...
SELECT
  ...
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN 1 ELSE 0 END) AS excel_row_count,
  count(DISTINCT CASE WHEN record_side = 'EXCEL_DETAIL' THEN imsi END) AS excel_distinct_imsi_count,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN excel_amount ELSE CAST(NULL AS DECIMAL(38,10)) END) AS excel_total_final_charge,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN platform_calculated_amount_known ELSE CAST(NULL AS DECIMAL(38,10)) END) AS platform_calculated_amount_known_total,
  CASE WHEN sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN 1 ELSE 0 END) = sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND platform_calculated_amount_known IS NOT NULL THEN 1 ELSE 0 END)
             AND sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND source_match_status = 'MATCHED' THEN 1 ELSE 0 END) = sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN 1 ELSE 0 END)
       THEN sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN platform_calculated_amount_known ELSE CAST(NULL AS DECIMAL(38,10)) END)
       ELSE CAST(NULL AS DECIMAL(38,10)) END AS platform_total,
  CASE WHEN sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN 1 ELSE 0 END) = sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND platform_calculated_amount_known IS NOT NULL THEN 1 ELSE 0 END)
             AND sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND source_match_status = 'MATCHED' THEN 1 ELSE 0 END) = sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN 1 ELSE 0 END) THEN 'COMPLETE' ELSE 'UNRESOLVED' END AS platform_total_status,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN amount_diff ELSE CAST(NULL AS DECIMAL(38,10)) END) AS amount_diff_total,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN abs(amount_diff) ELSE CAST(NULL AS DECIMAL(38,10)) END) AS amount_abs_diff_total,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN platform_calculated_days ELSE CAST(NULL AS DECIMAL(38,10)) END) AS platform_days_total,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND platform_calculated_days IS NOT NULL THEN 1 ELSE 0 END) AS platform_days_population,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND platform_calculated_price IS NOT NULL THEN 1 ELSE 0 END) AS platform_price_population,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND platform_calculated_amount IS NOT NULL THEN 1 ELSE 0 END) AS platform_amount_population,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND numeric_match_status = 'NUMERIC_MATCH' THEN 1 ELSE 0 END) AS numeric_match_row_count,
  count(DISTINCT CASE WHEN record_side = 'EXCEL_DETAIL' AND numeric_match_status = 'NUMERIC_MATCH' THEN imsi END) AS numeric_match_distinct_imsi_count,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND source_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS formal_matched_row_count,
  count(DISTINCT CASE WHEN record_side = 'EXCEL_DETAIL' AND source_match_status = 'MATCHED' THEN imsi END) AS formal_matched_distinct_imsi_count,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND source_match_status = 'NUMERIC_MATCH_ONLY' THEN 1 ELSE 0 END) AS numeric_match_only_row_count,
  count(DISTINCT CASE WHEN record_side = 'EXCEL_DETAIL' AND source_match_status = 'NUMERIC_MATCH_ONLY' THEN imsi END) AS numeric_match_only_distinct_imsi_count,
  sum(CASE WHEN source_match_status = 'WA_ONLY' THEN 1 ELSE 0 END) AS wa_only_row_count,
  count(DISTINCT CASE WHEN source_match_status = 'WA_ONLY' THEN imsi END) AS wa_only_distinct_imsi_count,
  sum(CASE WHEN source_match_status = 'PLATFORM_ONLY' THEN 1 ELSE 0 END) AS platform_only_row_count,
  count(DISTINCT CASE WHEN source_match_status = 'PLATFORM_ONLY' THEN imsi END) AS platform_only_distinct_imsi_count,
  sum(CASE WHEN source_match_status = 'UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_row_count,
  count(DISTINCT CASE WHEN source_match_status = 'UNRESOLVED' THEN imsi END) AS unresolved_distinct_imsi_count,
  sum(CASE WHEN source_match_status = 'MISSING_DATA' THEN 1 ELSE 0 END) AS missing_data_row_count,
  count(DISTINCT CASE WHEN source_match_status = 'MISSING_DATA' THEN imsi END) AS missing_data_distinct_imsi_count,
  sum(CASE WHEN source_match_status = 'MISSING_MAPPING' THEN 1 ELSE 0 END) AS missing_mapping_row_count,
  count(DISTINCT CASE WHEN source_match_status = 'MISSING_MAPPING' THEN imsi END) AS missing_mapping_distinct_imsi_count
FROM recon_joined
GROUP BY reconciliation_billing_month
ORDER BY reconciliation_billing_month