...
SELECT
  ...
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN platform_calculated_amount_known ELSE CAST(NULL AS DECIMAL(38,10)) END) -
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN excel_amount ELSE CAST(NULL AS DECIMAL(38,10)) END) AS known_amount_diff,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN amount_diff ELSE CAST(NULL AS DECIMAL(38,10)) END) AS candidate_row_amount_diff_total,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' THEN abs(amount_diff) ELSE CAST(NULL AS DECIMAL(38,10)) END) AS candidate_row_amount_abs_diff_total,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND platform_product_mapping_status = 'PROVEN_BY_PLATFORM_IMSI' THEN 1 ELSE 0 END) AS proven_mapping_row_count,
  count(DISTINCT CASE WHEN record_side = 'EXCEL_DETAIL' AND platform_product_mapping_status = 'PROVEN_BY_PLATFORM_IMSI' THEN imsi END) AS proven_mapping_distinct_imsi_count,
  ...
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND abs(day_diff) <= 0.0001 THEN 1 ELSE 0 END) AS day_exact_row_count,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND abs(price_diff) <= 0.0001 THEN 1 ELSE 0 END) AS price_exact_row_count,
  sum(CASE WHEN record_side = 'EXCEL_DETAIL' AND abs(amount_diff) <= 0.0001 THEN 1 ELSE 0 END) AS amount_exact_row_count,
