...
SELECT billing_month, reconciliation_status, mapping_status, platform_mapping_status,
  COUNT(*) AS card_count, ROUND(SUM(COALESCE(wa_total_amount, 0)), 6) AS wa_amount_sum,
  SUM(CASE WHEN detail_rows > 0 THEN 1 ELSE 0 END) AS wa_detail_cards,
  SUM(CASE WHEN pending_rows > 0 THEN 1 ELSE 0 END) AS wa_pending_cards,
  SUM(CASE WHEN status_snapshot_rows > 0 THEN 1 ELSE 0 END) AS platform_status_cards,
  SUM(CASE WHEN cycle_rows > 0 THEN 1 ELSE 0 END) AS platform_cycle_cards,
  SUM(CASE WHEN status_log_rows > 0 THEN 1 ELSE 0 END) AS platform_status_log_cards
FROM recon
GROUP BY billing_month, reconciliation_status, mapping_status, platform_mapping_status
ORDER BY billing_month, reconciliation_status, mapping_status, platform_mapping_status