-- C batch v3 hard-gate supplement: platform keys are target-month snapshot anchored.
-- supplier_id=2275 and product_id scope are retained; cycle/status logs only enrich scoped cards.
...
cycle_scope_rows AS (
  ...
  JOIN status_scope ss
    ON ss.billing_month = c.billing_month
   AND ss.imsi = CAST(h.imsi AS STRING)
  JOIN product_dim p
...
SELECT
  billing_month, imsi, charge_type, source_layer, excel_product_id, excel_product_names,
  excel_final_start_date, excel_final_end_date, excel_final_days, excel_final_days_values,
  excel_monthly_rate, excel_monthly_rate_values, excel_final_charge, excel_row_count,
  ...
  CASE WHEN platform_calculated_amount IS NOT NULL THEN excel_final_charge - platform_calculated_amount END AS platform_amount_diff,
  platform_price_effective_status, platform_scope_status, candidate_status,
  platform_scope_basis, source_files, sheet_names
FROM (
  SELECT
    ...
    'supplier_id=2275 + product_id + target_month_snapshot; price from product_dim.package_price; cycle_history.package_price retained' AS platform_scope_basis,
