...
, pending_raw AS (
  SELECT c.billing_month, CAST(x.imsi AS STRING) AS imsi,
    ...
    CASE WHEN COUNT(DISTINCT TRY_CAST(x.monthly_rate AS DOUBLE)) = 1 THEN MIN(TRY_CAST(x.monthly_rate AS DOUBLE)) END AS monthly_rate,
    ROUND(SUM(TRY_CAST(x.final_charge_for_usage_days AS DOUBLE)), 6) AS final_charge,
    ...
    CONCAT_WS(' || ', SORT_ARRAY(COLLECT_SET(CAST(x.monthly_rate AS STRING)))) AS monthly_rate_values,
...
SELECT billing_month, charge_type,
  ROUND(SUM(CASE WHEN source_layer = 'WA_DETAIL' THEN COALESCE(excel_final_charge, 0) ELSE 0 END), 6) AS excel_amount,
  ROUND(SUM(CASE WHEN source_layer = 'WA_DETAIL' THEN COALESCE(platform_calculated_amount, 0) ELSE 0 END), 6) AS platform_calculated_amount,
  ROUND(SUM(CASE WHEN source_layer = 'WA_DETAIL' AND platform_calculated_amount IS NOT NULL THEN excel_final_charge - platform_calculated_amount ELSE 0 END), 6) AS amount_diff,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_days IS NOT NULL THEN 1 ELSE 0 END) AS days_evaluable,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_days IS NOT NULL AND excel_final_days IS NOT NULL AND ABS(excel_final_days - platform_calculated_days) <= 0.01 THEN 1 ELSE 0 END) AS days_exact,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_days IS NOT NULL AND excel_final_days IS NOT NULL AND ABS(excel_final_days - platform_calculated_days) <= 0.01 THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_days IS NOT NULL THEN 1 ELSE 0 END), 0) AS days_exact_rate,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL THEN 1 ELSE 0 END) AS price_evaluable,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL AND ABS(excel_monthly_rate - platform_calculated_price) <= 0.01 THEN 1 ELSE 0 END) AS price_exact,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL AND ABS(excel_monthly_rate - platform_calculated_price) <= 0.01 THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN source_layer = 'WA_DETAIL' AND excel_monthly_rate IS NOT NULL AND platform_calculated_price IS NOT NULL THEN 1 ELSE 0 END), 0) AS price_exact_rate,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_amount IS NOT NULL THEN 1 ELSE 0 END) AS amount_evaluable,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_amount IS NOT NULL AND ABS(excel_final_charge - platform_calculated_amount) <= 0.01 THEN 1 ELSE 0 END) AS amount_exact,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_amount IS NOT NULL AND ABS(excel_final_charge - platform_calculated_amount) <= 0.01 THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_amount IS NOT NULL THEN 1 ELSE 0 END), 0) AS amount_exact_rate,
  SUM(CASE WHEN candidate_status IN ('MISSING_PLATFORM_DAY_EVENT', 'MISSING_PLATFORM_PRICE', 'PENDING_MISSING_FINAL_DAYS') THEN 1 ELSE 0 END) AS unresolved_card_count,
  ROUND(SUM(CASE WHEN candidate_status IN ('MISSING_PLATFORM_DAY_EVENT', 'MISSING_PLATFORM_PRICE', 'PENDING_MISSING_FINAL_DAYS') THEN COALESCE(excel_final_charge, 0) ELSE 0 END), 6) AS unresolved_excel_amount,
  SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_amount IS NOT NULL AND ABS(excel_final_charge - platform_calculated_amount) > 0.01 THEN 1 ELSE 0 END) AS residual_card_count,
  ROUND(SUM(CASE WHEN source_layer = 'WA_DETAIL' AND charge_type NOT LIKE 'Credit%' AND platform_calculated_amount IS NOT NULL AND ABS(excel_final_charge - platform_calculated_amount) > 0.01 THEN excel_final_charge - platform_calculated_amount ELSE 0 END), 6) AS residual_amount_diff
FROM (
  SELECT
    billing_month, imsi, charge_type, 'WA_DETAIL' AS source_layer,
    excel_product_id, excel_product_names, excel_final_start_date, excel_final_end_date,
    excel_final_days, excel_final_days_values, excel_monthly_rate, excel_monthly_rate_values,
    excel_final_charge, excel_row_count,
    platform_product_ids, platform_product_names, history_package_prices, history_price_count,
    dim_package_prices, dim_price_count, platform_calculated_days_rule, platform_calculated_days,
    excel_minus_platform_days, platform_calculated_price, excel_minus_platform_price,
    platform_calculated_amount, excel_calculated_amount,
    CAST(NULL AS DOUBLE) AS platform_amount_diff,
    CASE WHEN platform_calculated_price IS NULL THEN 'MISSING_PLATFORM_PRICE_OR_MAPPING'
         ELSE 'PRODUCT_ID_TARGET_WINDOW_NO_VALID_TO_HISTORY' END AS platform_price_effective_status,
    CASE WHEN platform_product_id_count IS NULL OR platform_product_id_count = 0 THEN 'NO_WA_PRODUCT_ID_FACT'
         ELSE 'WING_ALPHA_PRODUCT_ID_SCOPE' END AS platform_scope_status,
    CASE WHEN charge_type LIKE 'Credit%' THEN 'EXCEL_CREDIT_RAW_NEGATIVE_PRESERVED'
         WHEN platform_calculated_days IS NULL THEN 'MISSING_PLATFORM_DAY_EVENT'
         WHEN platform_calculated_price IS NULL THEN 'MISSING_PLATFORM_PRICE'
         WHEN platform_calculated_amount IS NULL THEN 'PLATFORM_CANDIDATE_NOT_CALCULABLE'
         WHEN ABS(excel_final_charge - platform_calculated_amount) <= 0.01 THEN 'PLATFORM_CANDIDATE_EXACT_AMOUNT'
         ELSE 'PLATFORM_CANDIDATE_AMOUNT_RESIDUAL' END AS candidate_status,
    'product_id + target_month_cycle_window; scope supplier_id=2275 + product_id target window; price from product_dim.package_price; cycle_history.package_price retained' AS platform_scope_basis,
    CAST(NULL AS STRING) AS source_files, CAST(NULL AS STRING) AS sheet_names
  FROM calculated
  UNION ALL
  SELECT
    p.billing_month, p.imsi, 'Pending Prorated-In' AS charge_type, 'WA_PENDING_PRORATED' AS source_layer,
    CAST(NULL AS STRING) AS excel_product_id, p.product_names AS excel_product_names,
    p.final_start_d AS excel_final_start_date, p.final_end_d AS excel_final_end_date,
    CAST(NULL AS DOUBLE) AS excel_final_days, CAST(NULL AS STRING) AS excel_final_days_values,
    p.monthly_rate AS excel_monthly_rate, p.monthly_rate_values AS excel_monthly_rate_values,
    p.final_charge AS excel_final_charge, p.pending_row_count AS excel_row_count,
    CAST(NULL AS STRING) AS platform_product_ids, CAST(NULL AS STRING) AS platform_product_names,
    CAST(NULL AS STRING) AS history_package_prices, CAST(NULL AS BIGINT) AS history_price_count,
    CAST(NULL AS STRING) AS dim_package_prices, CAST(NULL AS BIGINT) AS dim_price_count,
    'PENDING_NO_FINAL_DAYS' AS platform_calculated_days_rule, CAST(NULL AS DOUBLE) AS platform_calculated_days,
    CAST(NULL AS DOUBLE) AS excel_minus_platform_days, CAST(NULL AS DOUBLE) AS platform_calculated_price,
    CAST(NULL AS DOUBLE) AS excel_minus_platform_price, CAST(NULL AS DOUBLE) AS platform_calculated_amount,
    CAST(NULL AS DOUBLE) AS excel_calculated_amount, CAST(NULL AS DOUBLE) AS platform_amount_diff,
    'NO_PLATFORM_PRICE_ATTEMPT' AS platform_price_effective_status,
    'PENDING_NO_PLATFORM_SCOPE' AS platform_scope_status,
    'PENDING_MISSING_FINAL_DAYS' AS candidate_status,
    'pending-prorated source; no final_days column' AS platform_scope_basis,
    p.source_files, p.sheet_names
  FROM pending_raw p
) q
