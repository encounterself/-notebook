...
platform_only_rows AS (
  SELECT l.imsi, l.product_id, l.cycle_start_date, l.cycle_end_date, l.platform_package_price, l.platform_price_evidence_status
  FROM platform_cycle_lifecycle l
  WHERE NOT EXISTS (
    SELECT 1 FROM excel_detail_invoice x
    WHERE x.imsi = l.imsi
      AND ((x.excel_cycle_start_date IS NOT NULL AND x.excel_cycle_end_date IS NOT NULL AND x.excel_cycle_start_date = l.cycle_start_date AND x.excel_cycle_end_date = l.cycle_end_date)
        OR (x.excel_final_start_date IS NOT NULL AND x.excel_final_end_date IS NOT NULL AND l.cycle_start_date <= x.excel_final_end_date AND l.cycle_end_date >= x.excel_final_start_date))
  )
)
SELECT
  'DETAIL_EXCEL_ROW' AS population_type,
  r.invoice_batch_label AS official_excel_billing_month,
  r.observed_start_month,
  COALESCE(r.charge_type, '<NULL>') AS charge_type,
  r.rule_id,
  r.source_match_status,
  r.numeric_match_status,
  r.difference_reason,
  COUNT(*) AS row_count,
  COUNT(DISTINCT r.imsi) AS imsi_count,
  SUM(r.excel_amount) AS excel_total,
  SUM(r.platform_calculated_amount) AS platform_calculated_total,
  SUM(r.amount_diff) AS amount_diff_total,
  SUM(CASE WHEN r.day_diff IS NOT NULL AND ABS(r.day_diff) <= CAST(0.000001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS days_exact_row_count,
  SUM(CASE WHEN r.price_diff IS NOT NULL AND ABS(r.price_diff) <= CAST(0.000001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS price_exact_row_count,
  SUM(CASE WHEN r.amount_diff IS NOT NULL AND ABS(r.amount_diff) <= CAST(0.00001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS amount_exact_row_count,
  SUM(CASE WHEN r.day_diff IS NULL THEN 1 ELSE 0 END) AS days_missing_row_count,
  SUM(CASE WHEN r.price_diff IS NULL THEN 1 ELSE 0 END) AS price_missing_row_count,
  SUM(CASE WHEN r.amount_diff IS NULL THEN 1 ELSE 0 END) AS amount_missing_row_count,
  SUM(CASE WHEN r.source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN COALESCE(r.excel_amount, CAST(0 AS DECIMAL(38,12))) ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_impact_excel_amount,
  SUM(CASE WHEN r.source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN ABS(COALESCE(r.amount_diff, CAST(0 AS DECIMAL(38,12)))) ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_abs_amount_diff
FROM reconciliation_rows r
GROUP BY r.invoice_batch_label, r.observed_start_month, COALESCE(r.charge_type, '<NULL>'), r.rule_id, r.source_match_status, r.numeric_match_status, r.difference_reason
UNION ALL
SELECT
  'PLATFORM_ONLY_LIFECYCLE' AS population_type,
  CAST(NULL AS STRING) AS official_excel_billing_month,
  DATE_FORMAT(p.cycle_start_date, 'yyyy-MM') AS observed_start_month,
  '<PLATFORM_ONLY>' AS charge_type,
  'R_PLATFORM_LIFECYCLE_NO_EXCEL_WINDOW' AS rule_id,
  'PLATFORM_ONLY' AS source_match_status,
  'NO_EXCEL' AS numeric_match_status,
  'NO_EXCEL_MATCHED_WINDOW' AS difference_reason,
  COUNT(*) AS row_count,
  COUNT(DISTINCT p.imsi) AS imsi_count,
  CAST(NULL AS DECIMAL(38,12)) AS excel_total,
  CAST(NULL AS DECIMAL(38,12)) AS platform_calculated_total,
  CAST(NULL AS DECIMAL(38,12)) AS amount_diff_total,
  CAST(NULL AS BIGINT) AS days_exact_row_count,
  CAST(NULL AS BIGINT) AS price_exact_row_count,
  CAST(NULL AS BIGINT) AS amount_exact_row_count,
  CAST(NULL AS BIGINT) AS days_missing_row_count,
  CAST(NULL AS BIGINT) AS price_missing_row_count,
  CAST(NULL AS BIGINT) AS amount_missing_row_count,
  CAST(NULL AS DECIMAL(38,12)) AS unresolved_impact_excel_amount,
  CAST(NULL AS DECIMAL(38,12)) AS unresolved_abs_amount_diff
FROM platform_only_rows p
GROUP BY DATE_FORMAT(p.cycle_start_date, 'yyyy-MM')
ORDER BY population_type, official_excel_billing_month, observed_start_month, charge_type, source_match_status, numeric_match_status;