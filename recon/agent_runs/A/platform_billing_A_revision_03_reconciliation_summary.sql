...
month_detail_summary AS (
  SELECT
    ...
    SUM(CASE WHEN platform_calculated_amount IS NULL THEN 1 ELSE 0 END) AS missing_platform_amount_row_count,
    SUM(CASE WHEN source_match_status <> 'MATCHED' OR numeric_match_status <> 'MATCHED' THEN excel_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_or_unproven_excel_amount
...
month_reconciliation AS (
  SELECT
    ...
    COALESCE(m.missing_platform_amount_row_count, 0) AS missing_platform_amount_row_count,
...
row_status_summary AS (
  SELECT
    ...
    SUM(CASE WHEN source_match_status <> 'MATCHED' OR numeric_match_status <> 'MATCHED' THEN excel_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_or_unproven_excel_amount
  FROM reconciliation_rows
  GROUP BY source_file, invoice_batch_label, observed_start_month, charge_type, source_match_status, numeric_match_status
),
imsi_status_base AS (
  SELECT
    source_file,
    ...
    SUM(CASE WHEN platform_calculated_amount IS NULL THEN 1 ELSE 0 END) AS missing_platform_amount_row_count
  FROM reconciliation_rows
  GROUP BY source_file, invoice_batch_label, imsi, source_match_status, numeric_match_status
),
imsi_status_summary AS (
  SELECT
    ...
    SUM(missing_platform_amount_row_count) AS missing_platform_amount_row_count,
    SUM(CASE WHEN source_match_status <> 'MATCHED' OR numeric_match_status <> 'MATCHED' THEN excel_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_or_unproven_excel_amount
  FROM imsi_status_base
  GROUP BY source_file, invoice_batch_label, source_match_status, numeric_match_status
),
platform_only_lifecycle AS (
  ...
  WHERE NOT EXISTS (
    ...
    WHERE x.imsi = p.imsi
      AND (
        (x.excel_cycle_start_date IS NOT NULL AND x.excel_cycle_end_date IS NOT NULL
         AND p.cycle_start_date = x.excel_cycle_start_date AND p.cycle_end_date = x.excel_cycle_end_date)
        OR
        (x.excel_final_start_date IS NOT NULL AND x.excel_final_end_date IS NOT NULL
         AND p.cycle_start_date <= x.excel_final_end_date AND p.cycle_end_date >= x.excel_final_start_date)
      )
  )
),
...
SELECT
  ...
  price_exact_count,
  amount_exact_count,
  missing_platform_amount_row_count,
  unresolved_or_unproven_excel_amount,
  'Fixed detail source_file batch; 2026-01 has no official detail source_file' AS difference_reason
FROM month_reconciliation
UNION ALL
SELECT
  'ROW_STATUS', 'invoice_row', 'simo_prod.mysql_cdc_sync.wa_invoice_detail',
  source_file, invoice_batch_label, observed_start_month, charge_type,
  source_match_status, numeric_match_status, row_count, distinct_imsi_count,
  excel_amount, platform_calculated_amount, amount_diff,
  days_exact_count, price_exact_count, amount_exact_count,
  missing_platform_amount_row_count, unresolved_or_unproven_excel_amount,
  'Detail invoice row grain; rule_id and difference_reason are retained in Candidate row SQL'
FROM row_status_summary
UNION ALL
SELECT
  'IMSI_STATUS', 'distinct_imsi_card', 'simo_prod.mysql_cdc_sync.wa_invoice_detail',
  source_file, invoice_batch_label, CAST(NULL AS STRING), CAST(NULL AS STRING),
  source_match_status, numeric_match_status, row_count, distinct_imsi_count,
  excel_amount, platform_calculated_amount, amount_diff,
  days_exact_count, price_exact_count, amount_exact_count,
  missing_platform_amount_row_count, unresolved_or_unproven_excel_amount,
  'Distinct IMSI grain; row_count is invoice rows represented by those cards'
FROM imsi_status_summary
UNION ALL
SELECT
  'PLATFORM_ONLY', 'platform_lifecycle_row', 'simo_prod.ods.resource_res_vsim_cycle_history',
  CAST(NULL AS STRING), official_excel_billing_month, observed_start_month,
  CAST(NULL AS STRING), 'PLATFORM_ONLY', 'NO_EXCEL', row_count, distinct_imsi_count,
  CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)),
  CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT),
  CAST(NULL AS DECIMAL(38,12)),
  'supplier_id=2275 product-scoped cycle_history; outside Excel detail match or no Jan-2026 Excel source_file'
FROM platform_only_summary
ORDER BY result_type, official_excel_billing_month, source_file, charge_type, source_match_status, numeric_match_status
