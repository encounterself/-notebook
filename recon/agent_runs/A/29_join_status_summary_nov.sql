...
SELECT billing_month, join_status, COUNT(*) AS joined_row_count, COUNT(DISTINCT imsi) AS distinct_card_count,
  ROUND(SUM(CASE WHEN xl_charge IS NOT NULL THEN xl_charge ELSE 0 END), 4) AS invoice_amount,
  ROUND(SUM(CASE WHEN platform_calculated_amount IS NOT NULL THEN platform_calculated_amount ELSE 0 END), 4) AS platform_calculated_amount
FROM detail
GROUP BY billing_month, join_status
ORDER BY billing_month, join_status;