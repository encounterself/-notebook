...
, charge_type_row_metrics AS (
  SELECT
    ...
    SUM(CASE WHEN numeric_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS numeric_matched_row_count,
    SUM(CASE WHEN source_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS formal_matched_row_count,
    COUNT(DISTINCT CASE WHEN source_match_status = 'WA_ONLY' THEN imsi END) AS wa_only_imsi_count,
    COUNT(DISTINCT CASE WHEN source_match_status = 'UNRESOLVED' THEN imsi END) AS unresolved_imsi_count,
    COUNT(DISTINCT CASE WHEN source_match_status = 'MISSING_DATA' THEN imsi END) AS missing_data_imsi_count,
    COUNT(DISTINCT CASE WHEN source_match_status = 'MISSING_MAPPING' THEN imsi END) AS missing_mapping_imsi_count,
...
charge_type_imsi_base AS (
  SELECT
    ...
    SUM(CASE WHEN numeric_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS numeric_matched_row_count,
    SUM(CASE WHEN source_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS formal_matched_row_count,
    MAX(CASE WHEN source_match_status = 'WA_ONLY' THEN 1 ELSE 0 END) AS has_wa_only,
    MAX(CASE WHEN source_match_status = 'UNRESOLVED' THEN 1 ELSE 0 END) AS has_unresolved,
    MAX(CASE WHEN source_match_status = 'MISSING_DATA' THEN 1 ELSE 0 END) AS has_missing_data,
    MAX(CASE WHEN source_match_status = 'MISSING_MAPPING' THEN 1 ELSE 0 END) AS has_missing_mapping
  FROM reconciliation_rows
  GROUP BY invoice_batch_label, source_file, charge_type, rule_id, imsi
),
charge_type_imsi_metrics AS (
  SELECT
    invoice_batch_label,
    source_file,
    charge_type,
    ...
    SUM(CASE WHEN numeric_matched_row_count = invoice_row_count THEN 1 ELSE 0 END) AS numeric_matched_imsi_count,
    SUM(CASE WHEN formal_matched_row_count = invoice_row_count THEN 1 ELSE 0 END) AS formal_matched_imsi_count,
...
SELECT
  ...
  numeric_matched_row_count,
  formal_matched_row_count,
  wa_only_imsi_count,
  ...
  CAST(numeric_matched_row_count AS DOUBLE) / NULLIF(row_count, 0) AS numeric_match_rate,
  CAST(formal_matched_row_count AS DOUBLE) / NULLIF(row_count, 0) AS formal_match_rate
...
SELECT
  ...
  numeric_matched_imsi_count,
  formal_matched_imsi_count,
  wa_only_imsi_count,
  unresolved_imsi_count,
  missing_data_imsi_count,
  missing_mapping_imsi_count,
  ...
  CAST(numeric_matched_imsi_count AS DOUBLE) / NULLIF(distinct_imsi_count, 0),
  CAST(formal_matched_imsi_count AS DOUBLE) / NULLIF(distinct_imsi_count, 0)
FROM charge_type_imsi_metrics
