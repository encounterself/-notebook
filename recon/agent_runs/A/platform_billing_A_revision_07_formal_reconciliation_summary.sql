...
excel_detail_invoice AS (
  ...
  WHERE e.invoice_batch_label IN ('2025-09', '2025-10 (v1)', '2025-10 (v2)')
),
...
platform_cycle_rows AS (
  ...
  WHERE CAST(h.cycle_time AS TIMESTAMP) < TIMESTAMP '2026-02-01 00:00:00'
...
platform_status_rows_20251011 AS (
  SELECT
...
platform_status_summary AS (
  SELECT
    imsi,
    product_id,
    COUNT(*) AS status_snapshot_row_count_20251011,
    COUNT(DISTINCT partition_time) AS status_snapshot_partition_count_20251011,
    COUNT(DISTINCT sim_status) AS distinct_sim_status_count_20251011,
    COUNT(DISTINCT dispatch_status) AS distinct_dispatch_status_count_20251011,
    MIN(partition_time) AS first_status_partition_time_20251011,
    MAX(partition_time) AS last_status_partition_time_20251011
  FROM platform_status_rows_20251011
...
platform_cycle_candidates_raw AS (
  SELECT
    ...
    s.status_snapshot_row_count_20251011,
    s.status_snapshot_partition_count_20251011,
    s.distinct_sim_status_count_20251011,
    s.distinct_dispatch_status_count_20251011,
    s.first_status_partition_time_20251011,
    s.last_status_partition_time_20251011,
...
, formal_reconciliation_rows AS (
  SELECT
    ...
    CASE
      ...
      WHEN r.platform_price_evidence_status <> 'ONE_PRICE' THEN 'MISSING_DATA'
      ...
      WHEN r.excel_product_id IS NULL THEN 'MISSING_MAPPING'
...
, target_batch_map AS (
  SELECT '2025-09' AS invoice_batch_label, '2025-09' AS expected_observed_start_month
  UNION ALL SELECT '2025-10 (v1)', '2025-10'
  UNION ALL SELECT '2025-10 (v2)', '2025-11'
  UNION ALL SELECT '2026-01', '2026-01'
),
month_detail_summary AS (
  SELECT
    source_file,
    invoice_batch_label,
    COUNT(*) AS row_count,
    ...
    CASE WHEN SUM(CASE WHEN formal_match_status = 'MATCHED' THEN 1 ELSE 0 END) = COUNT(*)
         THEN SUM(platform_calculated_amount_known) END AS platform_total,
    SUM(amount_diff) AS amount_diff,
    SUM(CASE WHEN platform_calculated_days IS NOT NULL AND excel_days IS NOT NULL AND ABS(platform_calculated_days - excel_days) <= CAST(0.000001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS days_exact_count,
    SUM(CASE WHEN platform_package_price IS NOT NULL AND excel_price IS NOT NULL AND ABS(platform_package_price - excel_price) <= CAST(0.000001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS price_exact_count,
    SUM(CASE WHEN platform_calculated_amount IS NOT NULL AND excel_amount IS NOT NULL AND ABS(platform_calculated_amount - excel_amount) <= CAST(0.00001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS amount_exact_count,
  ...
  FROM formal_reconciliation_rows
  GROUP BY source_file, invoice_batch_label
),
month_reconciliation AS (
  SELECT
    t.invoice_batch_label,
    t.expected_observed_start_month,
    m.source_file,
    COALESCE(m.row_count, 0) AS row_count,
    COALESCE(m.distinct_imsi_count, 0) AS distinct_imsi_count,
    COALESCE(m.excel_amount, CAST(0 AS DECIMAL(38,12))) AS excel_amount,
    m.platform_calculated_amount_known,
    m.platform_total,
    m.amount_diff,
    COALESCE(m.days_exact_count, 0) AS days_exact_count,
    COALESCE(m.price_exact_count, 0) AS price_exact_count,
    COALESCE(m.amount_exact_count, 0) AS amount_exact_count,
    COALESCE(m.numeric_match_row_count, 0) AS numeric_match_row_count,
    COALESCE(m.formal_match_row_count, 0) AS formal_match_row_count,
    COALESCE(m.unknown_platform_amount_row_count, 0) AS unknown_platform_amount_row_count,
    COALESCE(m.unresolved_or_unproven_excel_amount, CAST(0 AS DECIMAL(38,12))) AS unresolved_or_unproven_excel_amount
  FROM target_batch_map t
  LEFT JOIN month_detail_summary m ON m.invoice_batch_label = t.invoice_batch_label
),
row_status_summary AS (
  SELECT
    source_file,
    invoice_batch_label,
    ...
    COUNT(DISTINCT imsi) AS distinct_imsi_count,
    SUM(excel_amount) AS excel_amount,
    SUM(platform_calculated_amount) AS platform_calculated_amount,
    SUM(platform_calculated_amount_known) AS platform_calculated_amount_known,
    SUM(amount_diff) AS amount_diff,
    SUM(CASE WHEN platform_calculated_days IS NOT NULL AND excel_days IS NOT NULL AND ABS(platform_calculated_days - excel_days) <= CAST(0.000001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS days_exact_count,
    SUM(CASE WHEN platform_package_price IS NOT NULL AND excel_price IS NOT NULL AND ABS(platform_package_price - excel_price) <= CAST(0.000001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS price_exact_count,
    ...
    SUM(CASE WHEN formal_match_status <> 'MATCHED' THEN excel_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_or_unproven_excel_amount
  FROM formal_reconciliation_rows
  GROUP BY source_file, invoice_batch_label, observed_start_month, charge_type, rule_id, numeric_match_status, formal_match_status, source_match_status
),
imsi_status_base AS (
  SELECT
    source_file,
    ...
    SUM(CASE WHEN platform_calculated_amount IS NOT NULL AND excel_amount IS NOT NULL AND ABS(platform_calculated_amount - excel_amount) <= CAST(0.00001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS amount_exact_count
  FROM formal_reconciliation_rows
  GROUP BY source_file, invoice_batch_label, imsi, numeric_match_status, formal_match_status, source_match_status
),
imsi_status_summary AS (
  SELECT
    source_file,
    invoice_batch_label,
    numeric_match_status,
    ...
    SUM(platform_calculated_amount_known) AS platform_calculated_amount_known,
    SUM(amount_diff) AS amount_diff,
    SUM(days_exact_count) AS days_exact_count,
    SUM(price_exact_count) AS price_exact_count,
    SUM(amount_exact_count) AS amount_exact_count,
    SUM(CASE WHEN formal_match_status <> 'MATCHED' THEN excel_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_or_unproven_excel_amount
  FROM imsi_status_base
  GROUP BY source_file, invoice_batch_label, numeric_match_status, formal_match_status, source_match_status
),
platform_only_lifecycle AS (
  SELECT
    p.imsi,
    p.product_id,
    p.cycle_start_date,
    p.cycle_end_date,
    DATE_FORMAT(p.cycle_start_date, 'yyyy-MM') AS observed_start_month,
    CASE DATE_FORMAT(p.cycle_start_date, 'yyyy-MM')
      WHEN '2025-09' THEN '2025-09'
      WHEN '2025-10' THEN '2025-10 (v1)'
      WHEN '2025-11' THEN '2025-10 (v2)'
      WHEN '2026-01' THEN '2026-01'
    END AS official_excel_billing_month
  FROM platform_cycle_lifecycle p
  WHERE NOT EXISTS (
    SELECT 1 FROM excel_detail_invoice x
    WHERE x.imsi = p.imsi
      AND ((x.excel_cycle_start_date IS NOT NULL AND x.excel_cycle_end_date IS NOT NULL AND p.cycle_start_date = x.excel_cycle_start_date AND p.cycle_end_date = x.excel_cycle_end_date)
        OR (x.excel_final_start_date IS NOT NULL AND x.excel_final_end_date IS NOT NULL AND p.cycle_start_date <= x.excel_final_end_date AND p.cycle_end_date >= x.excel_final_start_date))
)),
platform_only_summary AS (
  SELECT official_excel_billing_month, observed_start_month, COUNT(*) AS row_count, COUNT(DISTINCT imsi) AS distinct_imsi_count
  FROM platform_only_lifecycle
  GROUP BY official_excel_billing_month, observed_start_month
)
SELECT
  'MONTH_RECONCILIATION' AS result_type,
  'official_month' AS grain,
  'simo_prod.mysql_cdc_sync.wa_invoice_detail' AS source_table,
  source_file,
  invoice_batch_label AS official_excel_billing_month,
  expected_observed_start_month AS observed_start_month,
  CAST(NULL AS STRING) AS charge_type,
  'N/A' AS numeric_match_status,
  'EXCEL_TRUTH' AS formal_match_status,
  'EXCEL_TRUTH' AS source_match_status,
  row_count,
  distinct_imsi_count,
  excel_amount,
  ...
  'platform_total is NULL unless every detail row is formally MATCHED' AS difference_reason
FROM month_reconciliation
UNION ALL
SELECT
  'ROW_STATUS', 'invoice_row', 'simo_prod.mysql_cdc_sync.wa_invoice_detail', source_file,
  invoice_batch_label, observed_start_month, charge_type, numeric_match_status, formal_match_status,
  source_match_status, row_count, distinct_imsi_count, excel_amount, platform_calculated_amount_known,
  CAST(NULL AS DECIMAL(38,12)), amount_diff, days_exact_count, price_exact_count, amount_exact_count,
  CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), unknown_platform_amount_row_count,
  unresolved_or_unproven_excel_amount, 'row grain; selected candidate amount retained separately from known amount'
FROM row_status_summary
UNION ALL
SELECT
  'IMSI_STATUS', 'distinct_imsi_card', 'simo_prod.mysql_cdc_sync.wa_invoice_detail', source_file,
  invoice_batch_label, CAST(NULL AS STRING), CAST(NULL AS STRING), numeric_match_status, formal_match_status,
  source_match_status, row_count, distinct_imsi_count, excel_amount, platform_calculated_amount_known,
  CAST(NULL AS DECIMAL(38,12)), amount_diff, days_exact_count, price_exact_count, amount_exact_count,
  CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT),
  unresolved_or_unproven_excel_amount, 'distinct IMSI grain; row_count is represented invoice rows'
FROM imsi_status_summary
UNION ALL
SELECT
  'PLATFORM_ONLY', 'platform_lifecycle_row', 'simo_prod.ods.resource_res_vsim_cycle_history', CAST(NULL AS STRING),
  official_excel_billing_month, observed_start_month, CAST(NULL AS STRING), 'NOT_APPLICABLE', 'PLATFORM_ONLY', 'PLATFORM_ONLY',
  row_count, distinct_imsi_count, CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)),
  CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS DECIMAL(38,12)),
  'supplier_id=2275 product-scoped lifecycle with no Excel detail window match'
FROM platform_only_summary
ORDER BY result_type, official_excel_billing_month, source_file, charge_type, formal_match_status, numeric_match_status
