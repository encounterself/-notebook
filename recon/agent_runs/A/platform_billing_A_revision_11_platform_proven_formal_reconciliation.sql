...
month_detail_summary AS (
  ...
  FROM platform_proven_formal_rows
...
row_status_summary AS (
  SELECT
    ...
    SUM(CASE WHEN formal_match_status <> 'MATCHED' THEN excel_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_or_unproven_excel_amount
  FROM platform_proven_formal_rows
  GROUP BY source_file, invoice_batch_label, observed_start_month, charge_type, rule_id, numeric_match_status, formal_match_status, formal_match_status
),
imsi_status_base AS (
  SELECT
    source_file,
    ...
    SUM(CASE WHEN platform_calculated_amount IS NOT NULL AND excel_amount IS NOT NULL AND ABS(platform_calculated_amount - excel_amount) <= CAST(0.00001 AS DECIMAL(38,12)) THEN 1 ELSE 0 END) AS amount_exact_count
  FROM platform_proven_formal_rows
  GROUP BY source_file, invoice_batch_label, imsi, numeric_match_status, formal_match_status, formal_match_status
),
imsi_status_summary AS (
  ...
  GROUP BY source_file, invoice_batch_label, numeric_match_status, formal_match_status, formal_match_status
),
...
SELECT
  ...
  formal_match_status, row_count, distinct_imsi_count, excel_amount, platform_calculated_amount_known,
...
ORDER BY result_type, official_excel_billing_month, source_file, charge_type, numeric_match_status
