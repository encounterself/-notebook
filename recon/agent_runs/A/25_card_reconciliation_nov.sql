...
SELECT
  ...
  CASE WHEN join_status = 'MATCHED'
          AND candidate_days IS NOT NULL
          AND platform_price_if_unique IS NOT NULL
       THEN ROUND(platform_price_if_unique / days_in_month * candidate_days, 4) END AS platform_calculated_amount,
  CASE WHEN join_status = 'MATCHED'
          AND candidate_days IS NOT NULL AND xl_days IS NOT NULL
       THEN candidate_days - xl_days END AS days_difference,
  CASE WHEN join_status = 'MATCHED'
          AND platform_price_if_unique IS NOT NULL AND xl_rate IS NOT NULL
       THEN platform_price_if_unique - xl_rate END AS price_difference,
  CASE WHEN join_status = 'MATCHED'
          AND candidate_days IS NOT NULL
          AND platform_price_if_unique IS NOT NULL
          AND xl_charge IS NOT NULL
  ...
  CASE WHEN join_status <> 'MATCHED' THEN 'NOT_APPLICABLE'
       WHEN candidate_start_d IS NULL OR candidate_end_d IS NULL THEN 'UNRESOLVED_MISSING_LIFECYCLE_FIELD'
       WHEN candidate_end_d < candidate_start_d THEN 'UNRESOLVED_INVALID_DATE_WINDOW'
       ELSE 'CANDIDATE_WINDOW_VALID' END AS candidate_window_status,
  CASE WHEN join_status <> 'MATCHED' THEN 'NOT_APPLICABLE'
...
FROM joined
ORDER BY COALESCE(imsi, ''), invoice_line_no;