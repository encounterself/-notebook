...
SELECT
  ...
  COUNT(DISTINCT r.imsi) AS detail_imsi_count,
  ...
  SUM(CASE WHEN r.numeric_match_status = 'MATCHED' THEN 1 ELSE 0 END) AS numeric_matched_row_count,
  SUM(CASE WHEN r.source_match_status = 'UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_row_count,
  SUM(CASE WHEN r.source_match_status = 'MISSING_MAPPING' THEN 1 ELSE 0 END) AS missing_mapping_row_count,
  SUM(CASE WHEN r.source_match_status = 'MISSING_DATA' THEN 1 ELSE 0 END) AS missing_data_row_count,
  SUM(CASE WHEN r.source_match_status = 'WA_ONLY' THEN 1 ELSE 0 END) AS wa_only_row_count,
...
GROUP BY r.invoice_batch_label, r.observed_start_month
