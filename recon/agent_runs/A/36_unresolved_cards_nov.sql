...
, detail AS (
  ...
  SELECT billing_month, imsi, charge_type, join_status, xl_days, xl_rate, xl_charge,
    ...
    day_validation_status, price_validation_status,
    CASE WHEN join_status = 'PLATFORM_ONLY' THEN 'PLATFORM_ONLY'
         WHEN join_status = 'WA_ONLY' THEN 'WA_ONLY'
         WHEN candidate_window_status = 'UNRESOLVED_INVALID_DATE_WINDOW' THEN 'UNRESOLVED_INVALID_DATE_WINDOW'
         WHEN day_validation_status = 'DAY_MISMATCH' THEN 'DAY_MISMATCH'
         WHEN day_validation_status = 'UNRESOLVED_MISSING_LIFECYCLE_FIELD' THEN 'UNRESOLVED_MISSING_LIFECYCLE_FIELD'
         WHEN price_validation_status = 'PRICE_MISMATCH_OBSERVED' THEN 'PRICE_MISMATCH_OBSERVED'
         WHEN price_validation_status = 'UNRESOLVED_MULTIPLE_PLATFORM_PRICES' THEN 'UNRESOLVED_MULTIPLE_PLATFORM_PRICES'
         WHEN price_validation_status = 'MISSING_MAPPING_OR_PRICE' THEN 'MISSING_MAPPING_OR_PRICE'
         ELSE 'OTHER' END AS issue_class
  FROM detail
)
SELECT * FROM issue_rows WHERE issue_class <> 'OTHER'
ORDER BY billing_month, issue_class, imsi, charge_type;