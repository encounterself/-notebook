-- Batch A / read-only platform candidate and full outer reconciliation.
-- Excel fields are the monthly target. Platform fields explain or reproduce them.
-- Platform scope is first limited by target month partitions and Wing Alpha supplier_id=2275.
WITH cfg AS (
  SELECT '2025-11' AS billing_month, 2025 AS y, 11 AS m,
         DATE('2025-11-01') AS month_start, DATE('2025-11-30') AS month_end,
         30 AS days_in_month,
...
, detail AS (
  SELECT joined.*,
    ...
    CASE WHEN join_status = 'MATCHED' AND candidate_days IS NOT NULL AND xl_days IS NOT NULL THEN candidate_days - xl_days END AS days_difference,
    CASE WHEN join_status = 'MATCHED' AND platform_price_if_unique IS NOT NULL AND xl_rate IS NOT NULL THEN platform_price_if_unique - xl_rate END AS price_difference,
...
SELECT billing_month, COALESCE(charge_type, '<PLATFORM_ONLY>') AS charge_type, join_status,
  COUNT(*) AS joined_row_count, COUNT(DISTINCT imsi) AS distinct_card_count,
  COUNT(DISTINCT CASE WHEN join_status = 'MATCHED' THEN imsi END) AS matched_card_count,
  COUNT(DISTINCT CASE WHEN join_status = 'MATCHED' AND day_validation_status = 'EXACT_DAY_MATCH' THEN imsi END) AS exact_day_card_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND day_validation_status = 'EXACT_DAY_MATCH' THEN 1 ELSE 0 END) AS exact_day_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND day_validation_status = 'DAY_MISMATCH' THEN 1 ELSE 0 END) AS day_mismatch_line_count,
  SUM(CASE WHEN day_validation_status = 'UNRESOLVED_MISSING_LIFECYCLE_FIELD' THEN 1 ELSE 0 END) AS missing_lifecycle_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND days_difference IS NOT NULL THEN ABS(days_difference) ELSE 0 END) AS sum_abs_day_error,
  SUM(CASE WHEN join_status = 'MATCHED' AND price_validation_status = 'PRICE_MATCH_OBSERVED' THEN 1 ELSE 0 END) AS price_match_observed_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND price_validation_status = 'PRICE_MISMATCH_OBSERVED' THEN 1 ELSE 0 END) AS price_mismatch_observed_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND price_validation_status = 'UNRESOLVED_MULTIPLE_PLATFORM_PRICES' THEN 1 ELSE 0 END) AS multiple_price_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND price_validation_status = 'MISSING_MAPPING_OR_PRICE' THEN 1 ELSE 0 END) AS missing_mapping_or_price_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND platform_price_status = 'UNRESOLVED_NO_HISTORICAL_PRICE_EFFECTIVE_CONDITION' THEN 1 ELSE 0 END) AS price_effective_condition_unresolved_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND candidate_window_status = 'UNRESOLVED_INVALID_DATE_WINDOW' THEN 1 ELSE 0 END) AS invalid_date_window_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND candidate_days_plus1_minus1 IS NOT NULL AND candidate_days_plus1_minus1 = xl_days THEN 1 ELSE 0 END) AS activation_plus1_minus1_exact_line_count,
  SUM(CASE WHEN join_status = 'MATCHED' AND candidate_days_activation_end_inclusive IS NOT NULL AND candidate_days_activation_end_inclusive = xl_days THEN 1 ELSE 0 END) AS activation_end_inclusive_exact_line_count,
  SUM(CASE WHEN xl_charge IS NOT NULL THEN xl_charge ELSE 0 END) AS invoice_amount,
  SUM(CASE WHEN platform_calculated_amount IS NOT NULL THEN platform_calculated_amount ELSE 0 END) AS platform_calculated_amount,
  SUM(CASE WHEN platform_calculated_amount IS NOT NULL THEN platform_calculated_amount ELSE 0 END) - SUM(CASE WHEN xl_charge IS NOT NULL THEN xl_charge ELSE 0 END) AS amount_difference
FROM detail
GROUP BY billing_month, COALESCE(charge_type, '<PLATFORM_ONLY>'), join_status
ORDER BY billing_month, charge_type, join_status;