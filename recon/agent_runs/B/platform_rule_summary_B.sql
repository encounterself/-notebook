...
SELECT
  ...
  SUM(platform_calculated_days) AS platform_days_known,
  ...
  SUM(CASE WHEN activation_date_distance=0 THEN 1 ELSE 0 END) AS activation_date_exact_count,
  SUM(CASE WHEN activation_date_distance=1 THEN 1 ELSE 0 END) AS activation_date_pm1_count,
  SUM(CASE WHEN activation_date_distance>1 THEN 1 ELSE 0 END) AS activation_date_residual_count,
  SUM(CASE WHEN platform_activation_event_date IS NULL AND excel_charge_type='New Activation: Prorated-in Charge' THEN 1 ELSE 0 END) AS activation_event_missing_count
FROM final_candidate
GROUP BY official_excel_billing_month, excel_charge_type, rule_id, source_match_status
ORDER BY billing_month, charge_type, rule_id, source_match_status;