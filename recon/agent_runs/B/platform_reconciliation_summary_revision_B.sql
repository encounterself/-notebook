...
detail_status_rows AS (
  SELECT
    f.official_excel_billing_month AS billing_month,
    f.imsi,
    f.excel_charge_type AS charge_type,
    f.source_match_status AS status,
    'invoice_row' AS grain,
    f.excel_amount,
    f.platform_calculated_amount,
    f.excel_days,
    f.platform_calculated_days,
    CASE WHEN f.day_diff IS NOT NULL AND ABS(CAST(f.day_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END AS day_exact,
    CASE WHEN f.day_diff IS NOT NULL THEN 1 ELSE 0 END AS day_comparable,
    CASE WHEN f.price_diff IS NOT NULL AND ABS(CAST(f.price_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END AS price_exact,
    CASE WHEN f.price_diff IS NOT NULL THEN 1 ELSE 0 END AS price_comparable,
    CASE WHEN f.amount_diff IS NOT NULL AND ABS(CAST(f.amount_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END AS amount_exact,
    CASE WHEN f.platform_calculated_amount IS NULL THEN 1 ELSE 0 END AS platform_amount_missing
  FROM final_candidate f
  UNION ALL
  SELECT
    p.billing_month, p.imsi, CAST('PLATFORM_ONLY' AS STRING), CAST('PLATFORM_ONLY' AS STRING), 'platform_candidate_row',
    CAST(NULL AS DECIMAL(38,12)), p.platform_calculated_amount,
    CAST(NULL AS DECIMAL(38,12)), p.platform_calculated_days,
    0, 0, 0, 0, 0,
    CASE WHEN p.platform_calculated_amount IS NULL THEN 1 ELSE 0 END
...
monthly_excel AS (
  SELECT official_excel_billing_month AS billing_month,
    COUNT(*) AS row_count, COUNT(DISTINCT imsi) AS distinct_imsi_count,
    SUM(excel_amount) AS excel_total_final_charge,
    SUM(platform_calculated_amount) AS platform_calculated_amount_known,
    SUM(platform_calculated_days) AS platform_total_days_known,
    SUM(excel_days) AS excel_total_days,
    SUM(CASE WHEN platform_calculated_amount IS NULL THEN 1 ELSE 0 END) AS platform_amount_missing_row_count,
    SUM(CASE WHEN day_diff IS NOT NULL AND ABS(CAST(day_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS day_exact_count,
    SUM(CASE WHEN day_diff IS NOT NULL THEN 1 ELSE 0 END) AS day_comparable_count,
    SUM(CASE WHEN price_diff IS NOT NULL AND ABS(CAST(price_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS price_exact_count,
    SUM(CASE WHEN price_diff IS NOT NULL THEN 1 ELSE 0 END) AS price_comparable_count,
    SUM(CASE WHEN amount_diff IS NOT NULL AND ABS(CAST(amount_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS amount_exact_count
  FROM final_candidate
  GROUP BY official_excel_billing_month
),
charge_summary AS (
  SELECT official_excel_billing_month AS billing_month, excel_charge_type AS charge_type,
    COUNT(*) AS row_count, COUNT(DISTINCT imsi) AS distinct_imsi_count,
    SUM(excel_amount) AS excel_total_final_charge,
    SUM(platform_calculated_amount) AS platform_calculated_amount_known,
    SUM(platform_calculated_days) AS platform_total_days_known,
    ...
    SUM(CASE WHEN amount_diff IS NOT NULL AND ABS(CAST(amount_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS amount_exact_count
  FROM final_candidate
  GROUP BY official_excel_billing_month, excel_charge_type
),
row_status_summary AS (
  SELECT billing_month, status,
    ...
    SUM(platform_calculated_days) AS platform_total_days_known,
    SUM(excel_days) AS excel_total_days,
    SUM(platform_amount_missing) AS platform_amount_missing_row_count,
    SUM(day_exact) AS day_exact_count, SUM(day_comparable) AS day_comparable_count,
    SUM(price_exact) AS price_exact_count, SUM(price_comparable) AS price_comparable_count,
    SUM(amount_exact) AS amount_exact_count
  FROM detail_status_rows
...
detail_imsi AS (
  SELECT official_excel_billing_month AS billing_month, imsi,
    ...
    SUM(platform_calculated_days) AS platform_total_days_known,
    SUM(excel_days) AS excel_total_days,
    SUM(CASE WHEN source_match_status='MATCHED' THEN 1 ELSE 0 END) AS matched_rows,
    SUM(CASE WHEN source_match_status='WA_ONLY' THEN 1 ELSE 0 END) AS wa_only_rows,
    SUM(CASE WHEN source_match_status='MISSING_DATA' THEN 1 ELSE 0 END) AS missing_data_rows,
    SUM(CASE WHEN source_match_status='MISSING_MAPPING' THEN 1 ELSE 0 END) AS missing_mapping_rows,
    SUM(CASE WHEN source_match_status='UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_rows,
    SUM(CASE WHEN day_diff IS NOT NULL AND ABS(CAST(day_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS day_exact_count,
    SUM(CASE WHEN day_diff IS NOT NULL THEN 1 ELSE 0 END) AS day_comparable_count,
    SUM(CASE WHEN price_diff IS NOT NULL AND ABS(CAST(price_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS price_exact_count,
    SUM(CASE WHEN price_diff IS NOT NULL THEN 1 ELSE 0 END) AS price_comparable_count,
...
imsi_status_summary AS (
  SELECT billing_month,
    CASE WHEN matched_rows=detail_row_count THEN 'MATCHED'
         WHEN wa_only_rows=detail_row_count THEN 'WA_ONLY'
         WHEN missing_data_rows>0 THEN 'MISSING_DATA'
         WHEN missing_mapping_rows>0 THEN 'MISSING_MAPPING'
         ELSE 'UNRESOLVED' END AS status,
    SUM(detail_row_count) AS row_count, COUNT(*) AS distinct_imsi_count,
    SUM(excel_total_final_charge) AS excel_total_final_charge,
    SUM(platform_calculated_amount_known) AS platform_calculated_amount_known,
    SUM(platform_total_days_known) AS platform_total_days_known,
    SUM(excel_total_days) AS excel_total_days,
    SUM(platform_amount_missing_row_count) AS platform_amount_missing_row_count,
    SUM(day_exact_count) AS day_exact_count, SUM(day_comparable_count) AS day_comparable_count,
    SUM(price_exact_count) AS price_exact_count, SUM(price_comparable_count) AS price_comparable_count,
...
population_card_summary AS (
  SELECT billing_month, population_status AS status,
    COUNT(*) AS row_count, COUNT(DISTINCT imsi) AS distinct_imsi_count
  FROM population_full_outer
  GROUP BY billing_month, population_status
),
summary_output AS (
  SELECT 'EXCEL_MONTH_SUMMARY' AS result_section, t.billing_month, t.billing_month AS invoice_batch_label,
    CAST(NULL AS STRING) AS source_file, CAST('ALL_DETAIL' AS STRING) AS charge_type, CAST('ALL_DETAIL' AS STRING) AS status,
    'invoice_row' AS grain,
    COALESCE(e.row_count,0) AS row_count, COALESCE(e.distinct_imsi_count,0) AS distinct_imsi_count,
    COALESCE(e.excel_total_final_charge,CAST(0 AS DECIMAL(38,12))) AS excel_total_final_charge,
    e.platform_calculated_amount_known,
    CASE WHEN e.platform_calculated_amount_known IS NOT NULL THEN e.platform_calculated_amount_known-e.excel_total_final_charge ELSE NULL END AS amount_diff_known,
    e.excel_total_days, e.platform_total_days_known, e.day_exact_count, e.day_comparable_count,
    e.price_exact_count, e.price_comparable_count, e.amount_exact_count, e.platform_amount_missing_row_count
  FROM target_months t LEFT JOIN monthly_excel e ON t.billing_month=e.billing_month
  UNION ALL
  SELECT 'CHARGE_TYPE_SUMMARY', billing_month, billing_month, CAST(NULL AS STRING), charge_type, 'ALL_DETAIL', 'invoice_row',
    row_count, distinct_imsi_count, excel_total_final_charge, platform_calculated_amount_known,
    CASE WHEN platform_calculated_amount_known IS NOT NULL THEN platform_calculated_amount_known-excel_total_final_charge ELSE NULL END,
    excel_total_days, platform_total_days_known, day_exact_count, day_comparable_count,
    price_exact_count, price_comparable_count, amount_exact_count, platform_amount_missing_row_count
  FROM charge_summary
  UNION ALL
  SELECT 'ROW_STATUS_SUMMARY', billing_month, billing_month, CAST(NULL AS STRING), CAST(NULL AS STRING), status, 'row_status',
    row_count, distinct_imsi_count, excel_total_final_charge, platform_calculated_amount_known,
    CASE WHEN platform_calculated_amount_known IS NOT NULL AND excel_total_final_charge IS NOT NULL THEN platform_calculated_amount_known-excel_total_final_charge ELSE NULL END,
    excel_total_days, platform_total_days_known, day_exact_count, day_comparable_count,
    price_exact_count, price_comparable_count, amount_exact_count, platform_amount_missing_row_count
  FROM row_status_summary
  UNION ALL
  SELECT 'IMSI_STATUS_SUMMARY', billing_month, billing_month, CAST(NULL AS STRING), CAST(NULL AS STRING), status, 'distinct_imsi_card',
    row_count, distinct_imsi_count, excel_total_final_charge, platform_calculated_amount_known,
    CASE WHEN platform_calculated_amount_known IS NOT NULL THEN platform_calculated_amount_known-excel_total_final_charge ELSE NULL END,
    excel_total_days, platform_total_days_known, day_exact_count, day_comparable_count,
    price_exact_count, price_comparable_count, amount_exact_count, platform_amount_missing_row_count
  ...
  UNION ALL
  SELECT 'POPULATION_CARD_SUMMARY', billing_month, billing_month, CAST(NULL AS STRING), CAST(NULL AS STRING), status, 'distinct_imsi_card',
    row_count, distinct_imsi_count, CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)),
    CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT)
  FROM population_card_summary
)
SELECT * FROM summary_output
ORDER BY result_section, billing_month, status, charge_type;