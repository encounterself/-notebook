...
SELECT * FROM (SELECT rl.*, ROW_NUMBER() OVER (ORDER BY official_excel_billing_month, imsi, platform_product_id, platform_source_table) AS platform_only_row_id FROM row_level rl WHERE row_type='PLATFORM_ONLY') q WHERE platform_only_row_id BETWEEN 5001 AND 10000 ORDER BY official_excel_billing_month, imsi, platform_product_id, platform_source_table;