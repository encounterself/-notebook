...
row_level_revision AS (
  SELECT
    'ROW_LEVEL_REVISION' AS result_section, 'invoice_row' AS row_grain,
    1 AS row_count, 1 AS distinct_imsi_count,
    f.excel_row_id, f.excel_source_file AS source_file,
    f.official_excel_billing_month AS invoice_batch_label,
    f.official_excel_billing_month, f.observed_start_month,
    ...
    f.activation_date_distance, f.cycle_best_candidate_count, f.cycle_best_candidate_product_count,
    f.cycle_best_candidate_product_ids, f.platform_price_value_count, f.platform_price_values,
    f.snapshot_product_count, f.snapshot_platform_product_ids, f.snapshot_lifecycle_status_values,
    f.snapshot_dispatch_status_values, f.excel_offstock_date,
    f.excel_new_card_imsi_replacement, f.excel_old_card_imsi_replaced
  FROM final_candidate f
  UNION ALL
  SELECT
    'ROW_LEVEL_REVISION', 'platform_candidate_row',
    1, 1, CAST(NULL AS BIGINT), CAST(NULL AS STRING),
    p.billing_month, p.billing_month, CAST(NULL AS STRING),
    ...
    CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS STRING), CAST(NULL AS BIGINT), CAST(NULL AS STRING),
    CAST(NULL AS BIGINT), CAST(NULL AS STRING), p.lifecycle_status_values, p.dispatch_status_values,
...
SELECT * FROM (SELECT rl.*, ROW_NUMBER() OVER (ORDER BY invoice_batch_label, imsi, platform_product_id, platform_source_table) AS platform_only_row_id FROM row_level_revision rl WHERE row_grain='platform_candidate_row') q WHERE platform_only_row_id BETWEEN 10001 AND 20000 ORDER BY invoice_batch_label, imsi, platform_product_id, platform_source_table;