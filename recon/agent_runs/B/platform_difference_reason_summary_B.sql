...
SELECT official_excel_billing_month AS billing_month, excel_charge_type AS charge_type,
  source_match_status, difference_reason, COUNT(*) AS row_count, COUNT(DISTINCT imsi) AS imsi_count,
  SUM(excel_amount) AS excel_amount, SUM(platform_calculated_amount) AS platform_calculated_amount_known,
  SUM(excel_days) AS excel_days, SUM(platform_calculated_days) AS platform_days_known
FROM final_candidate
GROUP BY official_excel_billing_month, excel_charge_type, source_match_status, difference_reason
ORDER BY billing_month, charge_type, source_match_status, difference_reason;