EXPLAIN WITH cfg AS (
...
), excel_imsis AS (SELECT DISTINCT billing_month, imsi FROM excel_card),seed_cycle AS (
  ...
  SELECT DISTINCT e.billing_month, e.imsi, CAST(s.product_id AS STRING) AS platform_product_id, 'month_end_snapshot' AS seed_source
  FROM excel_imsis e JOIN cfg c ON c.billing_month = e.billing_month
  JOIN simo_prod.tc_cdr.sim_status_for_sftp_bak s
    ON s.imsi = e.imsi AND s.year = c.snap_year AND s.month = c.snap_month AND s.day = c.snap_day
), seed_product_ids AS (
  ...
  SELECT DISTINCT c.billing_month, CAST(s.imsi AS STRING) AS imsi, CAST(s.product_id AS STRING) AS platform_product_id,
         'tc_cdr.sim_status_for_sftp_bak' AS source_table, CAST(s.partition_time AS DATE) AS snapshot_date
  FROM cfg c JOIN simo_prod.tc_cdr.sim_status_for_sftp_bak s
    ON s.year = c.snap_year AND s.month = c.snap_month AND s.day = c.snap_day
  JOIN seed_product_ids k ON k.billing_month = c.billing_month AND k.platform_product_id = CAST(s.product_id AS STRING)
...
void_ranked AS (
  ...
  SELECT billing_month, imsi, COUNT(*) AS candidate_line_count,
         ROUND(SUM(excel_amount), 6) AS candidate_excel_amount,
         ROUND(SUM(COALESCE(platform_calculated_amount, 0)), 6) AS candidate_platform_calculated_amount,
         ROUND(SUM(excel_days), 6) AS candidate_excel_days,
         ROUND(SUM(COALESCE(platform_calculated_days, 0)), 6) AS candidate_platform_days,
         SUM(CASE WHEN day_match = 1 THEN 1 ELSE 0 END) AS day_match_count,
         SUM(CASE WHEN price_match = 1 THEN 1 ELSE 0 END) AS price_match_count,
         SUM(CASE WHEN population_status = 'UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_line_count,
         ROUND(SUM(CASE WHEN population_status = 'UNRESOLVED' THEN excel_amount ELSE 0 END), 6) AS unresolved_excel_impact,
         CONCAT_WS(' | ', SORT_ARRAY(COLLECT_SET(CASE WHEN population_status = 'UNRESOLVED' THEN difference_reason END))) AS unresolved_reasons,
         CONCAT_WS(' | ', SORT_ARRAY(COLLECT_SET(charge_type))) AS candidate_charge_types
  FROM candidate_line GROUP BY billing_month, imsi
), recon AS (
  SELECT COALESCE(e.billing_month, p.billing_month) AS billing_month,
         COALESCE(e.imsi, p.imsi) AS imsi,
         CASE WHEN e.imsi IS NOT NULL AND p.imsi IS NOT NULL THEN 'MATCHED'
              WHEN e.imsi IS NOT NULL THEN 'WA_ONLY' ELSE 'PLATFORM_ONLY' END AS reconciliation_status,
         e.excel_line_count, e.excel_charge_type_count, e.excel_charge_types, e.excel_product_names,
         CAST(NULL AS STRING) AS excel_product_id, e.excel_price_values, e.excel_price, e.excel_amount AS excel_amount,
         e.excel_days AS excel_days, e.excel_min_start_date, e.excel_max_end_date,
         p.platform_product_ids AS platform_product_id, p.platform_product_names, p.platform_supplier_ids, p.platform_source_tables,
         p.platform_calculated_price AS platform_price, p.platform_calculated_days AS platform_days,
         CASE WHEN e.imsi IS NOT NULL AND p.imsi IS NOT NULL THEN cc.candidate_platform_calculated_amount
              WHEN e.imsi IS NULL AND p.imsi IS NOT NULL AND p.platform_calculated_price IS NOT NULL THEN p.platform_calculated_price * p.platform_calculated_days / DAY(c.month_end)
              ELSE NULL END AS platform_calculated_amount,
         CASE WHEN e.imsi IS NOT NULL AND p.imsi IS NOT NULL THEN ROUND(cc.candidate_platform_calculated_amount - e.excel_amount, 6)
              WHEN e.imsi IS NULL AND p.imsi IS NOT NULL AND p.platform_calculated_price IS NOT NULL THEN ROUND(p.platform_calculated_price * p.platform_calculated_days / DAY(c.month_end), 6)
              ELSE NULL END AS amount_diff,
         CASE WHEN e.imsi IS NOT NULL AND p.imsi IS NOT NULL THEN cc.day_match_count ELSE NULL END AS day_match_count,
         CASE WHEN e.imsi IS NOT NULL AND p.imsi IS NOT NULL THEN cc.price_match_count ELSE NULL END AS price_match_count,
         CASE WHEN e.imsi IS NOT NULL AND p.imsi IS NOT NULL THEN cc.unresolved_line_count ELSE NULL END AS unresolved_line_count,
         CASE WHEN e.imsi IS NOT NULL AND p.imsi IS NOT NULL THEN cc.unresolved_excel_impact ELSE CAST(0 AS DOUBLE) END AS unresolved_excel_impact,
         COALESCE(cc.unresolved_reasons, p.platform_calculated_amount_rule) AS difference_reason,
         COALESCE(cc.candidate_charge_types, '<PLATFORM_ONLY>') AS charge_types,
         p.platform_scope_reason,
         COALESCE(p.excluded_platform_product_ids, ex.excluded_platform_product_ids) AS excluded_platform_product_ids,
         COALESCE(p.excluded_platform_product_names, ex.excluded_platform_product_names) AS excluded_platform_product_names,
         COALESCE(p.excluded_supplier_ids, ex.excluded_supplier_ids) AS excluded_supplier_ids,
         COALESCE(p.excluded_reasons, ex.excluded_reasons) AS excluded_reasons,
         COALESCE(p.excluded_source_tables, ex.excluded_source_tables) AS excluded_source_tables,
         CASE WHEN p.imsi IS NOT NULL THEN 'KEEP_WA: supplier_id=2275 AND product_name contains (WA); product_id seeded from target Excel IMSI cycle/snapshot; product_name is not a join key'
              WHEN ex.imsi IS NOT NULL THEN 'No KEEP_WA platform card; excluded observed product_id(s) are listed with supplier and reason'
              ELSE 'No scoped platform product record' END AS platform_scope_basis
  FROM excel_card e FULL OUTER JOIN platform_cards p ON p.billing_month = e.billing_month AND p.imsi = e.imsi
  LEFT JOIN candidate_card cc ON cc.billing_month = COALESCE(e.billing_month, p.billing_month) AND cc.imsi = COALESCE(e.imsi, p.imsi)
  LEFT JOIN cfg c ON c.billing_month = COALESCE(e.billing_month, p.billing_month)
  LEFT JOIN platform_excluded_summary ex ON ex.billing_month = COALESCE(e.billing_month, p.billing_month) AND ex.imsi = COALESCE(e.imsi, p.imsi)
)
SELECT billing_month, imsi, reconciliation_status,
       excel_product_id, excel_product_names AS excel_product_name, excel_price_values AS excel_price, excel_amount, excel_days,
       platform_product_id, platform_product_names AS platform_product_name, platform_supplier_ids, platform_price, platform_calculated_amount, platform_days,
       amount_diff, day_match_count AS day_match, price_match_count AS price_match,
       excel_line_count, excel_charge_type_count, charge_types, unresolved_line_count, unresolved_excel_impact, difference_reason,
       platform_scope_reason, platform_scope_basis, platform_source_tables,
       excluded_platform_product_ids, excluded_platform_product_names, excluded_supplier_ids, excluded_reasons, excluded_source_tables,
       excel_min_start_date, excel_max_end_date
FROM recon
ORDER BY billing_month, reconciliation_status, imsi