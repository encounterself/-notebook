...
charge_summary AS (
  SELECT
    official_excel_billing_month AS billing_month,
    excel_charge_type AS charge_type,
    COUNT(*) AS excel_row_count,
    COUNT(DISTINCT imsi) AS excel_imsi_count,
    SUM(excel_amount) AS official_excel_total_final_charge,
    SUM(excel_days) AS excel_total_days,
    SUM(platform_calculated_days) AS platform_total_days_known,
    SUM(platform_calculated_amount) AS platform_calculated_amount_known,
    SUM(platform_calculated_amount)-SUM(excel_amount) AS amount_diff_known,
    SUM(CASE WHEN day_diff IS NOT NULL AND ABS(CAST(day_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS day_match_count,
    SUM(CASE WHEN day_diff IS NOT NULL THEN 1 ELSE 0 END) AS day_comparable_count,
    SUM(CASE WHEN price_diff IS NOT NULL AND ABS(CAST(price_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS price_match_count,
    SUM(CASE WHEN price_diff IS NOT NULL THEN 1 ELSE 0 END) AS price_comparable_count,
    SUM(CASE WHEN amount_diff IS NOT NULL AND ABS(CAST(amount_diff AS DOUBLE))<=0.000001 THEN 1 ELSE 0 END) AS amount_match_count,
    SUM(CASE WHEN source_match_status='MATCHED' THEN 1 ELSE 0 END) AS matched_row_count,
    SUM(CASE WHEN source_match_status='WA_ONLY' THEN 1 ELSE 0 END) AS wa_only_row_count,
    SUM(CASE WHEN source_match_status='UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_row_count,
    SUM(CASE WHEN source_match_status='MISSING_DATA' THEN 1 ELSE 0 END) AS missing_data_row_count,
    SUM(CASE WHEN source_match_status='MISSING_MAPPING' THEN 1 ELSE 0 END) AS missing_mapping_row_count,
    SUM(CASE WHEN source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN excel_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_excel_amount_impact,
    SUM(CASE WHEN source_match_status IN ('UNRESOLVED','MISSING_DATA','MISSING_MAPPING','WA_ONLY') THEN platform_calculated_amount ELSE CAST(0 AS DECIMAL(38,12)) END) AS unresolved_platform_amount_known
  FROM final_candidate
  GROUP BY official_excel_billing_month, excel_charge_type
),
month_population_summary AS (
  SELECT
    billing_month,
    SUM(CASE WHEN population_status='MATCHED' THEN 1 ELSE 0 END) AS matched_population_card_count,
    SUM(CASE WHEN population_status='WA_ONLY' THEN 1 ELSE 0 END) AS wa_only_population_card_count,
    SUM(CASE WHEN population_status='PLATFORM_ONLY' THEN 1 ELSE 0 END) AS platform_only_population_card_count,
    SUM(COALESCE(excel_row_count,0)) AS excel_row_count,
    SUM(COALESCE(excel_amount,CAST(0 AS DECIMAL(38,12)))) AS official_excel_total_final_charge,
    SUM(COALESCE(excel_row_platform_amount_known,CAST(0 AS DECIMAL(38,12)))) AS excel_rows_platform_amount_known,
    SUM(COALESCE(platform_amount_impact_known,CAST(0 AS DECIMAL(38,12)))) AS platform_calculated_amount_known,
    SUM(COALESCE(platform_amount_impact_known,CAST(0 AS DECIMAL(38,12))))-SUM(COALESCE(excel_amount,CAST(0 AS DECIMAL(38,12)))) AS amount_diff_known,
    SUM(CASE WHEN population_status='PLATFORM_ONLY' THEN COALESCE(platform_amount_impact_known,CAST(0 AS DECIMAL(38,12))) ELSE CAST(0 AS DECIMAL(38,12)) END) AS platform_only_amount_impact_known,
    SUM(CASE WHEN population_status='PLATFORM_ONLY' THEN COALESCE(platform_unresolved_amount_row_count,0) ELSE 0 END) AS platform_only_unresolved_amount_row_count,
    SUM(COALESCE(unresolved_row_count,0)) AS unresolved_excel_row_count,
    SUM(COALESCE(missing_data_row_count,0)) AS missing_data_excel_row_count,
    SUM(COALESCE(missing_mapping_row_count,0)) AS missing_mapping_excel_row_count,
    SUM(COALESCE(wa_only_row_count,0)) AS wa_only_excel_row_count
  FROM population_full_outer
  GROUP BY billing_month
),
summary_output AS (
  SELECT
    'CHARGE_TYPE_SUMMARY' AS result_section, billing_month, charge_type,
    excel_row_count, excel_imsi_count,
    official_excel_total_final_charge, excel_total_days, platform_total_days_known,
    platform_calculated_amount_known, amount_diff_known,
    day_match_count, day_comparable_count,
    CASE WHEN day_comparable_count>0 THEN CAST(day_match_count AS DOUBLE)/day_comparable_count ELSE NULL END AS day_match_rate,
    price_match_count, price_comparable_count,
    CASE WHEN price_comparable_count>0 THEN CAST(price_match_count AS DOUBLE)/price_comparable_count ELSE NULL END AS price_match_rate,
    amount_match_count, matched_row_count, wa_only_row_count, unresolved_row_count, missing_data_row_count, missing_mapping_row_count,
    unresolved_excel_amount_impact, unresolved_platform_amount_known,
    CAST(NULL AS BIGINT) AS matched_population_card_count,
    CAST(NULL AS BIGINT) AS wa_only_population_card_count,
    CAST(NULL AS BIGINT) AS platform_only_population_card_count,
    CAST(NULL AS DECIMAL(38,12)) AS platform_only_amount_impact_known,
    CAST(NULL AS BIGINT) AS platform_only_unresolved_amount_row_count,
    CAST(NULL AS BIGINT) AS unresolved_excel_row_count,
    CAST(NULL AS BIGINT) AS missing_data_excel_row_count,
    CAST(NULL AS BIGINT) AS missing_mapping_excel_row_count,
    CAST(NULL AS BIGINT) AS wa_only_excel_row_count
  FROM charge_summary
  UNION ALL
  SELECT
    'MONTH_POPULATION_SUMMARY', billing_month, CAST(NULL AS STRING),
    excel_row_count, CAST(NULL AS BIGINT), official_excel_total_final_charge, CAST(NULL AS DECIMAL(38,12)), CAST(NULL AS DECIMAL(38,12)),
    platform_calculated_amount_known, amount_diff_known,
    CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS DOUBLE), CAST(NULL AS BIGINT), CAST(NULL AS BIGINT), CAST(NULL AS DOUBLE), CAST(NULL AS BIGINT),
    ...
    matched_population_card_count, wa_only_population_card_count, platform_only_population_card_count,
    platform_only_amount_impact_known, platform_only_unresolved_amount_row_count,
    unresolved_excel_row_count, missing_data_excel_row_count, missing_mapping_excel_row_count, wa_only_excel_row_count
  FROM month_population_summary
)
SELECT * FROM summary_output
ORDER BY result_section, billing_month, charge_type;