...
void_ranked AS (
  ...
  SELECT billing_month, charge_type, 'EXCEL_LINE_POPULATION' AS population,
         COUNT(*) AS line_count, COUNT(DISTINCT imsi) AS card_count,
         ROUND(SUM(excel_amount), 6) AS excel_amount,
         ROUND(SUM(COALESCE(platform_calculated_amount, 0)), 6) AS platform_calculated_amount,
         ROUND(SUM(COALESCE(platform_calculated_amount, 0)) - SUM(excel_amount), 6) AS amount_diff,
         ROUND(SUM(excel_days), 6) AS excel_days, ROUND(SUM(COALESCE(platform_calculated_days, 0)), 6) AS platform_days,
         ...
         SUM(CASE WHEN population_status = 'CALCULATED' THEN 1 ELSE 0 END) AS calculated_count,
         SUM(CASE WHEN population_status = 'UNRESOLVED' THEN 1 ELSE 0 END) AS unresolved_count,
         ...
         CONCAT_WS(' | ', SORT_ARRAY(COLLECT_SET(CASE WHEN population_status = 'UNRESOLVED' THEN CAST(imsi AS STRING) END))) AS unresolved_sample_imsis
  FROM candidate_line GROUP BY billing_month, charge_type
), platform_only_summary AS (
  SELECT p.billing_month, '<PLATFORM_ONLY>' AS charge_type, 'PLATFORM_CARD_POPULATION' AS population,
         0 AS line_count, COUNT(*) AS card_count, CAST(0 AS DOUBLE) AS excel_amount,
         ROUND(SUM(CASE WHEN p.platform_calculated_price IS NOT NULL THEN p.platform_calculated_price * p.platform_calculated_days / DAY(c.month_end) ELSE 0 END), 6) AS platform_calculated_amount,
         ROUND(SUM(CASE WHEN p.platform_calculated_price IS NOT NULL THEN p.platform_calculated_price * p.platform_calculated_days / DAY(c.month_end) ELSE 0 END), 6) AS amount_diff,
         CAST(0 AS DOUBLE) AS excel_days, ROUND(SUM(p.platform_calculated_days), 6) AS platform_days,
         CAST(NULL AS BIGINT) AS day_match_count, CAST(NULL AS BIGINT) AS price_match_count,
         SUM(CASE WHEN p.platform_calculated_price IS NOT NULL THEN 1 ELSE 0 END) AS calculated_count,
         SUM(CASE WHEN p.platform_calculated_price IS NULL THEN 1 ELSE 0 END) AS unresolved_count,
         CAST(0 AS DOUBLE) AS unresolved_excel_impact,
         CONCAT_WS(' | ', SORT_ARRAY(COLLECT_SET(CASE WHEN p.platform_calculated_price IS NULL THEN COALESCE(p.platform_calculated_amount_rule, 'MISSING_PLATFORM_PRICE') END))) AS unresolved_reasons,
         CONCAT_WS(' | ', SORT_ARRAY(COLLECT_SET(CASE WHEN p.platform_calculated_price IS NULL THEN p.imsi END))) AS unresolved_sample_imsis
  FROM platform_cards p JOIN cfg c ON c.billing_month = p.billing_month
  LEFT JOIN excel_card e ON e.billing_month = p.billing_month AND e.imsi = p.imsi
  WHERE e.imsi IS NULL GROUP BY p.billing_month
)
SELECT * FROM candidate_summary
UNION ALL SELECT * FROM platform_only_summary
ORDER BY billing_month, population, charge_type